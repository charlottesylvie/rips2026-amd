#include "delta_stepping_hip_CSR_threads_v2.hpp"

// Keep the V2 implementation linkable alongside the original threaded
// implementation.  The copied core below uses the original internal names;
// remap them only in this translation unit.
#define ds_delta_threads_detail ds_delta_threads_v2_detail
#define DeltaSteppingThreadsCsrProgress DeltaSteppingThreadsV2CsrProgress
#define DeltaSteppingThreadsCsrProgressCallback DeltaSteppingThreadsV2CsrProgressCallback
#define DeltaSteppingThreadsCsrResult DeltaSteppingThreadsV2CsrResult
#define DeltaSteppingThreadsCsrWorkspace DeltaSteppingThreadsV2CsrWorkspace
#define delta_stepping_threads_minplus_hip_csr delta_stepping_threads_v2_minplus_hip_csr
#define delta_stepping_threads_hip_csr delta_stepping_threads_v2_hip_csr
...
#ifndef DS_V2_HIGH_DEGREE_THRESHOLD
#define DS_V2_HIGH_DEGREE_THRESHOLD 16
#endif
constexpr Offset kHighDegreeThreshold = DS_V2_HIGH_DEGREE_THRESHOLD;
#if defined(__HIP_PLATFORM_AMD__)
constexpr int kHybridWavefrontSize = 64;
#else
constexpr int kHybridWavefrontSize = 32;
#endif
static_assert(kBlockSize % kHybridWavefrontSize == 0,
              "block size must be a whole number of wavefronts");

// V2 keeps one thread per low-degree vertex and uses a wavefront for vertices
// with more than kHighDegreeThreshold outgoing edges.
...
struct DeltaSteppingScratch {
  ...
  DeviceBuffer<int> high_degree_queue;
  ...
  DeviceBuffer<int> high_degree_count;
  ...
  explicit DeltaSteppingScratch(Offset rows_)
      : rows(rows_),
        ...
        high_degree_queue(static_cast<std::size_t>(rows_)),
...
inline int grid_for_wavefront_frontier(int items) {
  if (items <= 0) return 1;
  constexpr int kWavefrontsPerBlock = kBlockSize / kHybridWavefrontSize;
  const long long blocks =
      (static_cast<long long>(items) + kWavefrontsPerBlock - 1) /
      kWavefrontsPerBlock;
...
__global__ void relax_light_edges_kernel(const int* frontier,
                                         ...
                                         int* heavy_queue,
                                         int* heavy_count,
                                         int* high_degree_queue,
                                         int* high_degree_count,
                                         int queue_capacity,
                                         int* overflow) {
#if DS_USE_LOWDEG_RELAX
  // One GPU thread handles the common low-degree case.  The companion
  // wavefront kernel below handles the remaining high-degree vertices.
  for (int fi = blockIdx.x * blockDim.x + threadIdx.x;
       fi < frontier_count;
       fi += blockDim.x * gridDim.x) {
    const int u = frontier[fi];
    const Offset begin = out_rowptr[u];
    const Offset end = out_rowptr[u + 1];
    if (end - begin > kHighDegreeThreshold) {
      const int pos = atomicAdd(high_degree_count, 1);
      if (pos < queue_capacity) {
        high_degree_queue[pos] = u;
      } else {
        atomicExch(overflow, 1);
      }
      continue;
...
// One wavefront cooperates on each high-degree active vertex.  This avoids a
// long single-lane edge loop without assigning an entire 256-thread block to
// the 25-37 degree tail seen in FPGA routing graphs.
__global__ void relax_light_edges_high_degree_kernel(
    const int* frontier,
  ...
  const int wavefront_index = global_thread / kHybridWavefrontSize;
  const int lane = threadIdx.x % kHybridWavefrontSize;
  const int wavefront_count = gridDim.x * (blockDim.x / kHybridWavefrontSize);
  const int frontier_count = *frontier_count_ptr;

  for (int fi = wavefront_index; fi < frontier_count; fi += wavefront_count) {
    const int u = frontier[fi];
    const Offset begin = out_rowptr[u];
    const Offset end = out_rowptr[u + 1];
    if (lane == 0) in_current[u] = 0;

    const float du = dist[u];
    if (!isfinite(du) || bucket_index(du, inv_delta) != current_bucket) continue;

    for (Offset e = begin + lane; e < end; e += kHybridWavefrontSize) {
      const float w = out_values[e];
      const int v = static_cast<int>(out_colind[e]);
      const float effective_w =
          vertex_costs == nullptr ? w : w * vertex_costs[v];
      if (effective_w > delta) {
        // The membership flag makes this safe when multiple lanes see heavy
        // edges of the same vertex.  Light-only vertices never enter the
        // heavy queue, so their adjacency is not scanned twice.
        enqueue_unique(u, in_heavy, heavy_queue, heavy_count, queue_capacity, overflow);
        continue;
      }
      const float nd = du + effective_w;
      ...
      if (nd < old) {
        ...
        if (b == current_bucket) {
          atomicExch(&in_pending[v], 0);
          enqueue_unique(v, in_current, next_frontier, next_count,
                         queue_capacity, overflow);
        } else if (b > current_bucket && b < kNoBucket) {
          enqueue_unique(v, in_pending, pending_queue, pending_count,
                         queue_capacity, overflow);
        }
      }
    }
...
__global__ void relax_heavy_edges_kernel(const int* heavy_vertices,
                                         ...
                                         int* overflow) {
#if DS_USE_LOWDEG_RELAX
  // One GPU thread handles the common low-degree heavy frontier.
  for (int fi = blockIdx.x * blockDim.x + threadIdx.x;
       fi < heavy_count_value;
       fi += blockDim.x * gridDim.x) {
    const int u = heavy_vertices[fi];
    if (out_rowptr[u + 1] - out_rowptr[u] > kHighDegreeThreshold) {
      const int pos = atomicAdd(high_degree_count, 1);
      if (pos < queue_capacity) {
        high_degree_queue[pos] = u;
      } else {
...
__global__ void relax_heavy_edges_high_degree_kernel(
    const int* heavy_vertices,
  ...
  for (int fi = wavefront_index; fi < heavy_count_value; fi += wavefront_count) {
    const int u = heavy_vertices[fi];
    const Offset begin = out_rowptr[u];
    const Offset end = out_rowptr[u + 1];
    if (lane == 0) in_heavy[u] = 0;
...
void initialize_scratch_once(DeltaSteppingScratch& scratch,
  ...
  if (scratch.initialized) {
    ...
    reset_int_zero_async(scratch.high_degree_count.get(), stream);
  ...
  for (int iter = 0; iter < max_iters; ++iter) {
    ...
    while (current_count > 0) {
      ...
      relax_light_edges_kernel<<<light_grid, kBlockSize, 0, stream>>>(
          current_queue, current_count,
          current_bucket, delta, inv_delta,
          ...
          scratch.heavy_count.get(), scratch.high_degree_queue.get(),
          scratch.high_degree_count.get(), n_int, scratch.overflow.get());
      DS_DELTA_HIP_CHECK(hipGetLastError());
#if DS_USE_LOWDEG_RELAX
      const int high_light_grid = grid_for_wavefront_frontier(current_count);
      relax_light_edges_high_degree_kernel<<<high_light_grid, kBlockSize, 0, stream>>>(
          scratch.high_degree_queue.get(), scratch.high_degree_count.get(),
          current_bucket, delta, inv_delta,
          d_adjacency.rowptr, d_adjacency.colind, d_adjacency.values,
          vertex_costs, scratch.dist.get(),
          scratch.in_current.get(), scratch.in_pending.get(), scratch.in_heavy.get(),
    ...
    if (heavy_count > 0) {
      ...
      relax_heavy_edges_kernel<<<heavy_grid, kBlockSize, 0, stream>>>(
          ...
          scratch.pending_count.get(), scratch.high_degree_queue.get(),
          scratch.high_degree_count.get(), n_int, scratch.overflow.get());
      DS_DELTA_HIP_CHECK(hipGetLastError());
#if DS_USE_LOWDEG_RELAX
      const int high_heavy_grid = grid_for_wavefront_frontier(heavy_count);
      relax_heavy_edges_high_degree_kernel<<<high_heavy_grid, kBlockSize, 0, stream>>>(
          scratch.high_degree_queue.get(), scratch.high_degree_count.get(),
          current_bucket, delta, inv_delta,
          d_adjacency.rowptr, d_adjacency.colind, d_adjacency.values,
          vertex_costs, scratch.dist.get(),
