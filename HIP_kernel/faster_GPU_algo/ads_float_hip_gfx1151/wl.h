...
static_assert(NUM_BAG == ADS_WARP_SIZE,
              "ADDS bucket metadata assumes one 32-lane wave");
...
__device__ __forceinline__ unsigned rs_set_per_assign_wrong(unsigned reg_stat, float per_assign_wrong) {
	...
	wrong = hd_min(MAX_EDGE_WRONG, wrong);
...
void worklist::alloc(int num_tb, int warp_per_tb, unsigned suggested_size, int nnode, int nedge) {
	...
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&delta_info_broadcast),
			sizeof(unsigned)));

	num_tb_real = num_tb;
	num_tb_32 = r_up(num_tb_real, 32);
	num_warp = warp_per_tb;
	const unsigned num_seg = wl_size / SEG_SIZE;
	const unsigned num_block = wl_size / BLOCK_SIZE;

	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&wl_data),
			static_cast<size_t>(wl_size) * sizeof(unsigned)));
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&write_done),
			static_cast<size_t>(num_seg) * sizeof(unsigned)));
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&resv_ptr),
			NUM_BAG * sizeof(unsigned)));
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&done_work_count),
			NUM_BAG * sizeof(unsigned)));
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&agm_buf),
			static_cast<size_t>(num_tb_real) * sizeof(unsigned long long)));
	HIP_CHECK(hipMemset(agm_buf, 0,
			static_cast<size_t>(num_tb_real) * sizeof(unsigned long long)));
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&regular_status),
			static_cast<size_t>(num_tb_32) * sizeof(unsigned)));

	for (int bag = 0; bag < NUM_BAG; ++bag) {
		HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&bk_ptr_array[bag]),
				static_cast<size_t>(num_block_linear) * sizeof(unsigned)));
	}
	HIP_CHECK(hipMalloc(reinterpret_cast<void**>(&read_done),
			static_cast<size_t>(num_block) * sizeof(unsigned)));
...
void worklist::reinit() {
	...
	HIP_CHECK(hipMemset(write_done, 0,
			static_cast<size_t>(num_seg) * sizeof(unsigned)));
	HIP_CHECK(hipMemset(resv_ptr, 0, NUM_BAG * sizeof(unsigned)));
	HIP_CHECK(hipMemset(done_work_count, 0,
			NUM_BAG * sizeof(unsigned)));
	HIP_CHECK(hipMemset(agm_buf, 0,
			static_cast<size_t>(num_tb_real) * sizeof(unsigned long long)));
	HIP_CHECK(hipMemset(delta_info_broadcast, 0, sizeof(unsigned)));

	unsigned reg_stat_host[MAX_TB];
	for (int i = 0; i < num_tb_32; ++i) {
		reg_stat_host[i] = i < num_tb_real ? 0x80000000u : 0u;
	}
	HIP_CHECK(hipMemcpy(regular_status, reg_stat_host,
			static_cast<size_t>(num_tb_32) * sizeof(unsigned),
			hipMemcpyHostToDevice));

	for (int bag = 0; bag < NUM_BAG; ++bag) {
		HIP_CHECK(hipMemset(bk_ptr_array[bag], 0xff,
				static_cast<size_t>(num_block_linear) * sizeof(unsigned)));
	}
	HIP_CHECK(hipMemset(read_done, 0,
			static_cast<size_t>(num_block) * sizeof(unsigned)));
}
...
void worklist::set_param(float ave_wt, float ave_degree) {
	...
	if (!(ave_wt > 0.0f) || !std::isfinite(ave_wt) ||
			!(ave_degree > 0.0f) || !std::isfinite(ave_degree)) {
		std::fprintf(stderr,
				"Invalid ADDS graph statistics: average weight=%g, "
				"average degree=%g\n",
				ave_wt, ave_degree);
		std::exit(EXIT_FAILURE);
	}

	// Pick the largest power-of-two work grain not exceeding 32/degree.
	const double suggested_grain = 32.0 / static_cast<double>(ave_degree);
	mini_grain = 1u;
	while (mini_grain < ADS_WARP_SIZE &&
			static_cast<double>(mini_grain << 1u) <= suggested_grain) {
		mini_grain <<= 1u;
	}

	quantum = ave_wt / static_cast<float>(1u << QUAN_SHIFT);
	if (!(quantum > 0.0f) || !std::isfinite(quantum)) {
		std::fprintf(stderr,
				"Invalid ADDS quantum derived from mean edge weight %g\n",
				ave_wt);
		std::exit(EXIT_FAILURE);
	}

	// In the original expression, ave_wt cancels algebraically.  Compute in
	// double precision and clamp the shift to the five-bit field carried by the
	// worklist broadcast.  This also avoids the original unsigned-power-of-two
	// loop overflowing on very sparse graphs.
	const double desired =
		(static_cast<double>(ave_wt) / static_cast<double>(ave_degree) /
		 static_cast<double>(INIT_FACTOR)) /
		static_cast<double>(quantum);
	int nearest_shift = 2;  // The original implementation enforces u >= 4.
	if (std::isfinite(desired) && desired > 0.0) {
		int lower_shift = static_cast<int>(std::floor(std::log2(desired)));
		lower_shift = hd_max(MIN_SHIFT, hd_min(MAX_SHIFT, lower_shift));
		const double lower = std::ldexp(1.0, lower_shift);
		if (lower_shift < MAX_SHIFT) {
			const double upper = std::ldexp(1.0, lower_shift + 1);
			nearest_shift =
				(desired - lower) < (upper - desired)
					? lower_shift
					: lower_shift + 1;
		} else {
			nearest_shift = MAX_SHIFT;
		}
	}
	delta_init = hd_max(2, hd_min(MAX_SHIFT, nearest_shift));

	std::printf(
			"multi-bag, multi-wave, HIP variant, wl size=%u, linear size=%u\n",
			wl_size, linear_size);
	std::printf(
			"MAX_CON_BAG=%u, MINI_GRAIN=%u, ave_wt=%f, ave_degree=%f, "
			"quantum=%g, delta_init=%d (desired=%g)\n",
			MAX_CON_BAG, mini_grain, ave_wt, ave_degree, quantum,
			delta_init, desired);
}

void worklist::free() {
	HIP_CHECK(hipFree(wl_data));
	HIP_CHECK(hipFree(write_done));
	HIP_CHECK(hipFree(resv_ptr));
	HIP_CHECK(hipFree(agm_buf));
	HIP_CHECK(hipFree(read_done));
	HIP_CHECK(hipFree(done_work_count));
	HIP_CHECK(hipFree(delta_info_broadcast));
	HIP_CHECK(hipFree(regular_status));
	for (int bag = 0; bag < NUM_BAG; ++bag) {
		HIP_CHECK(hipFree(bk_ptr_array[bag]));
		bk_ptr_array[bag] = nullptr;
	}
	wl_data = nullptr;
	write_done = nullptr;
	read_done = nullptr;
	resv_ptr = nullptr;
	agm_buf = nullptr;
	regular_status = nullptr;
	done_work_count = nullptr;
	delta_info_broadcast = nullptr;
}

#define MB_CACHE_SIZE 16
__shared__ volatile unsigned manager_exit;
__shared__ volatile unsigned cur_bag_mtb;
__shared__ volatile unsigned read_ptr[NUM_BAG];
__shared__ volatile unsigned assign_ptr[NUM_BAG];
__shared__ volatile unsigned manager_bk_cache[NUM_BAG][MB_CACHE_SIZE];
__shared__ unsigned shift_delta_local;
__shared__ volatile unsigned start_signal;
...
__device__ void worklist::alloc_manager() {
	...
	while (manager_exit == 0) {
		...
		unsigned resv_bk = (global_load(&(resv_ptr[m_bag]))) / BLOCK_SIZE;
		...
		unsigned alloc_mask = warp_ballot_sync(FULL_MASK, need_alloc);
		...
		for (unsigned pos = 1; find_nth_bit(alloc_mask, 0, pos) != NOT_FOUND; pos++) {
			...
			for (int i = get_lane_id(); i < num_block_word; i += 32) {
				...
				unsigned vote_result = warp_ballot_sync(FULL_MASK, (bit_idx != NOT_FOUND));
				...
				if (lane_idx != NOT_FOUND) {
					...
					if (lane_idx == get_lane_id()) {
						...
						global_store(&(bk_ptr_array[bag_id][alloc_idx]), ptr);
						//set the bit
		...
		}
		{
			...
			unsigned real_bk_idx = global_load(&(bk_ptr_array[m_bag][done_bk])) / BLOCK_SIZE;
			unsigned done_count = global_load(&(read_done[real_bk_idx]));
			bool done = (done_count == BLOCK_SIZE);
			//handle all read done
			unsigned done_mask = warp_ballot_sync(FULL_MASK, done);
			num_alloc_bk -= count_bit(done_mask);
			while (done_mask != 0) {
				unsigned target_bag = find_ms_bit(done_mask);
				unsigned target_real_bk_idx = warp_shfl(FULL_MASK, real_bk_idx, target_bag);
				...
				for (int seg_idx = get_lane_id(); seg_idx < (BLOCK_SIZE / SEG_SIZE); seg_idx += 32) {
					...
					global_store(&(write_done[seg_word]), (unsigned) 0);
				}
			...
			if (done) {
				...
				global_store(&(read_done[real_bk_idx]), (unsigned) 0);
				global_store(&(bk_ptr_array[m_bag][done_bk]), (unsigned) NOT_FOUND);
		...
		if (((num_block - num_alloc_bk)) < (NUM_BAG)) {
			if (get_lane_id() == 0) {
				printf("running out of worklist memory; increase ADDS_WL_ENTRIES\n");
			}
			assert(false);
		}
		...
		if ((m_read_ptr - assign_ptr[m_bag]) < (total_threads * 2)) {
			...
			if (cur_tag == m_tag) {
				...
				for (; lower_line_ptr < BLOCK_SIZE; lower_line_ptr += (32 * SEG_SIZE)) {
					unsigned count = global_load(&(write_done[m_word]));
					unsigned vote_result = warp_ballot_sync(FULL_MASK, (count != SEG_SIZE));
					if (vote_result != 0) {
						word_offset = find_nth_bit(vote_result, 0, 1);
						sub_count = warp_shfl(FULL_MASK, count, word_offset);
						break;
					}
					m_word += 32;
				}
				...
				if (new_lower_ptr <= read_lower_ptr) {
					...
					__threadfence();
					cur_resv_ptr = global_load(&(resv_ptr[m_bag]));
					cur_done_work = global_load(&(done_work_count[m_bag]));
				...
				if (get_lane_id() < MB_CACHE_SIZE) {
					...
					unsigned real_ptr = global_load(&(bk_ptr_array[m_bag][bk_array_idx]));
...
__device__ __forceinline__ int inc_delta_shift(int shift, int amount) {
	return hd_min(MAX_SHIFT, shift + amount);
}
__device__ __forceinline__ int dec_delta_shift(int shift, int amount) {
	return hd_max(MIN_SHIFT, shift - amount);
}
...
//////////////////////////
	...
	probe_low_work = hd_max(PROBE_LOW_WORK_MIN, probe_low_work);
	...
	if (ave_degree >= UTL_LINEAR_TSH) {
		...
		low_P = CURVE_LOW_P_COEFF * powf(ave_degree, CURVE_LOW_P_POW);
		low_UTL = CURVE_LOW_UTL_COEFF * powf(ave_degree, CURVE_LOW_UTL_POW);
		hi_P = CURVE_HI_P_COEFF * powf(ave_degree, CURVE_HI_P_POW);
		hi_UTL = CURVE_HI_UTL_COEFF * powf(ave_degree, CURVE_HI_UTL_POW);
	}

	unsigned cur_state = STATE_RESET;
	int last_inc_delta_bag = -10;
	int last_dec_delta_bag = -10;
	unsigned remaining_work = 0;
	unsigned in_range_assign = 0;
	bool invite_check = false;
	bool start_state_machine = false;
	float last_config_util = 0.0f;
	float transit_util = 0.0f;
	float forgive_count = 0.0f;
//push start node
	...
	unsigned small_start_bag = 0;
	unsigned small_start_work = 0;
		...
		if (last_assign_ptr[updated_bag % NUM_BAG] < read_ptr[updated_bag % NUM_BAG]) {
			if (!fixed_delta) {
				if ((prof_assigned_tb >= measure_tb) || ((current_bag != updated_bag))) {
					...
					unsigned new_resv_ptr = global_load(&(resv_ptr[m_bag]));
					unsigned produced_work = new_resv_ptr - old_resv_ptr;
					unsigned warp_produced_work = warp_reduce_sum(produced_work);
					warp_produced_work = warp_shfl(FULL_MASK, warp_produced_work, 0);
					unsigned cur_assign_ptr = assign_ptr[m_bag];
					unsigned all_work = new_resv_ptr - cur_assign_ptr;
					//update bag dist
					{
						if (warp_produced_work > 0) {
							unsigned m_dist = cur_dist + get_lane_id() * (1u << cur_delta);
					...
					unsigned A_sum = warp_reduce_sum(A_count);
					unsigned P_sum = warp_reduce_sum(P_count);
					A_sum = warp_shfl(FULL_MASK, A_sum, 0);
					P_sum = warp_shfl(FULL_MASK, P_sum, 0);
					...
					if (!start_state_machine) {
						...
						if (total_assigned_work > init_assign_tsh) {
							...
							coeff = hd_min(MAX_SM_TB, coeff);
							coeff = hd_max(MIN_SM_TB, coeff);
					...
					if (((prof_iter - last_change_delta_iter) > CHANGE_DELTA_WAIT) && (cur_delta == next_delta)) {
						if (probe_low_phase) {
							if ((updated_bag - current_bag) >= (NUM_BAG - 1)) {
								...
								unsigned high_clip_mask = warp_ballot_sync(FULL_MASK, m_percent > HIGH_CLIP);
									...
									if ((cur_P < hi_P) && (util < hi_UTL)) {
										...
										if (util < transit_util) {
											...
											if (forgive_count > DEC_DELTA_FORGIVE) {
												...
												remaining_work = warp_reduce_sum(cur_in_range_work);
												remaining_work = warp_shfl(FULL_MASK, remaining_work, 0);
					...
					old_resv_ptr = warp_shfl(FULL_MASK, new_resv_ptr, (get_lane_id() + (updated_bag - current_bag)) % 32);
					prof_assigned_tb = 0;
			...
			for (unsigned i = get_lane_id(); i < num_tb_32; i += 32) {
				...
				unsigned m_status = global_load(&(regular_status[m_tb]));
				unsigned regular_seq = rs_get_seq(m_status);
				bool avail_tb = (regular_seq != local_seq[m_tb]);
				unsigned avail_tb_mask = warp_ballot_sync(FULL_MASK, avail_tb);
			...
			if (probe_low_phase) {
				//clip to 1
				total_avail_tb = hd_min(total_avail_tb, 1);
			}
			...
			//add them up
			unsigned total_avail_work = warp_reduce_sum(m_avail_work);
			total_avail_work = warp_shfl(FULL_MASK, total_avail_work, 0);
			...
			if ((total_avail_tb > 0) && (total_avail_work > 0)) {
				...
				per_warp_work = hd_max(per_warp_work, mini_grain);
				...
				per_warp_work = hd_min(per_warp_work, 32);
				...
				while (cur_assigned_tb != total_avail_tb) {
					...
					unsigned m_translated_assign_ptr = 0;
					unsigned m_assign_ptr = assign_ptr[bag_id_map];
					if (get_lane_id() < concur_bag) {
						unsigned m_read_ptr = read_ptr[bag_id_map];
					...
					if (probe_low_phase) {
						//cap to low
						m_num_work = hd_min(m_num_work, probe_low_work);
					}
					//Available bags
					bool availbe_bag = (m_num_work > 0);
					unsigned availbe_bag_mask = warp_ballot_sync(FULL_MASK, availbe_bag);
					...
					if (availbe_bag_mask != 0) {
						...
						unsigned num_work = warp_shfl(FULL_MASK, m_num_work, bag_lane);
						...
						unsigned real_assign_ptr = warp_shfl(FULL_MASK, m_translated_assign_ptr, bag_lane);
						...
						if (valid) {
							...
							local_seq[m_tb] = seq;
							//get tb from lst
							global_store(&(agm_buf[m_tb]), m_assignment);
							new_work[m_tb] = m_size;

						}

						//update assign ptr, from the last valid assignment
						unsigned last_lane = find_ms_bit(warp_ballot_sync(FULL_MASK, valid));
						unsigned assigned_work = warp_shfl(FULL_MASK, m_offset + m_size, last_lane);
			...
			for (int m_tb = get_lane_id(); m_tb < num_tb_real; m_tb += 32) {
				global_store(&(agm_buf[m_tb]), EXIT);
			}
		}
...
__device__ void worklist::tb_coop_process(CSRGraph& graph, int warp_id) {
	...
	if (work_size > 0) {
		...
		for (int offset = get_lane_id(); offset < work_size; offset += 32) {
			...
			node_data_type new_dist = global_load(&(graph.node_data[vertex_id])) + wt;
			node_data_type dst_dist = global_load(&(graph.node_data[dst]));
...
__device__ __forceinline__ void worklist::tb_coop_assign(int vertex_id, int first_edge, unsigned long long m_assignment, int& m_size, unsigned tb_coop_lane) {
	...
	unsigned lock_val = 0;
	if (get_lane_id() == 0) {
		lock_val = atomicCAS(&tb_coop_lock, 0u, 1u);
	}
	lock_val = warp_shfl(FULL_MASK, lock_val, 0);
	if (lock_val == 0) {
		//grabbed lock, do the assignment
		//find free buffers
		bool free_buffer = (tb_coop_work_size[get_lane_id()] == 0);
		unsigned free_mask = warp_ballot_sync(FULL_MASK, free_buffer);
		...
		if (free_count > MIN_TB_COOP_WARP) {
			...
			unsigned leader_vertex_id = warp_shfl(FULL_MASK, vertex_id, tb_coop_lane);
			unsigned leader_first_edge = warp_shfl(FULL_MASK, first_edge, tb_coop_lane);
			unsigned leader_size = warp_shfl(FULL_MASK, m_size, tb_coop_lane);

			unsigned per_warp_work = (leader_size / free_count) + 1;
			//round up to 32
			per_warp_work = roundup(per_warp_work, 32);
			per_warp_work = hd_max(MIN_TB_COOP_PER_WARP_WORK, per_warp_work);

			//calc my assignment
			unsigned m_start = per_warp_work * m_offset;
			unsigned m_end = hd_min(leader_size, m_start + per_warp_work);
			...
			unsigned valid_count = count_bit(warp_ballot_sync(FULL_MASK, valid));
			if (get_lane_id() == 0) {
...
//poll global status
	unsigned lock_val = 0;
	if (get_lane_id() == 0) {
		lock_val = atomicCAS(&tb_lock, 0u, 1u);
	}
	lock_val = warp_shfl(FULL_MASK, lock_val, 0);
	if (lock_val == 0) {
		unsigned delta_info = 0;
		if (get_lane_id() == 0) {
			delta_info = global_load(delta_info_broadcast);
		}
		warp_sync();
...
//use rr for load balancing
		...
		unsigned avail_mask = warp_ballot_sync(FULL_MASK, warp_avail);
...
//it is possible to have non-availble warp (this warp can be assigned while here)
		if (num_avail > 0) {
			...
			if (tb_assginment == 0) {
				//check global
				if (get_lane_id() == 0) {
					tb_assginment = global_load(&(agm_buf[block_id_x()]));
				}
				//find free counter
				...
				unsigned vote_result = warp_ballot_sync(FULL_MASK, has_free_counter);
				free_counter_idx = find_ms_bit(vote_result);
				tb_assginment = warp_shfl(FULL_MASK, tb_assginment, 0);
			...
			unsigned valid_mask = warp_ballot_sync(FULL_MASK, valid);
			if (valid) {
		...
		if (get_lane_id() == 0) {
			...
			global_store(&(regular_status[block_id_x()]), m_status);

			//unlock
			__threadfence_block();
			atomicExch(&tb_lock, 0u);
		}
		warp_sync();
	}
}
...
//check my assignment
		...
		m_assignment = warp_shfl(FULL_MASK, m_assignment, 0);
		if (m_assignment == 0) {
			//poll global status
			get_global(warp_id);
		} else if (m_assignment == EXIT) {
			// All lanes in the wave observe EXIT together.  CUDA used a PTX
			// per-thread exit instruction here; HIP returns to the kernel instead.
...
//try update
	if (atomicCAS(&(bK_cache_lock[bag_id]), 0u, 1u) == 0) {
		unsigned bk_array_idx = m_tag & (num_block_linear - 1);	//Virtual bk idx
		unsigned real_ptr = global_load(&(bk_ptr_array[bag_id][bk_array_idx]));
		if (real_ptr != NOT_FOUND) {
			//now we can update the cache
			unsigned real_idx = real_ptr / BLOCK_SIZE;
			unsigned entry = 0;
		...
		__threadfence_block();
//unlock
		atomicExch(&bK_cache_lock[bag_id], 0u);
	}
}
...
__device__ __forceinline__ void worklist::push_work(unsigned bag_id, unsigned node) {
	unsigned active_mask = warp_active_mask();
	unsigned match_mask = warp_match_any(active_mask, bag_id);
	...
	r_ptr = warp_shfl(active_mask, r_ptr, leader);
...
//set the done counter
	unsigned seg_mask = warp_match_any(active_mask, m_seg);
	leader = find_ms_bit(seg_mask);
	global_store(&(wl_data[write_ptr]), node);
	__threadfence();
	if (get_lane_id() == leader) {
...
__device__ __forceinline__ unsigned worklist::pop_work(unsigned idx) {
	return global_load(&(wl_data[idx]));
}

__device__ __forceinline__ unsigned worklist::dist_to_bag_id_int(
		unsigned bag_id, node_data_type dst_dist) {
	const unsigned delta_info = delta_info_local;
	const unsigned delta = bfg_get_delta(delta_info);
	const unsigned cur_bag = bfg_get_bag(delta_info);
	const unsigned cur_dist_shift = bfg_get_dist(delta_info);

	// Avoid undefined float-to-int overflow for very long paths.  Values beyond
	// the representable quantized range naturally belong in the furthest bag.
	const float scaled = dst_dist / quantum;
	// 2^32 is exactly representable as float; the next lower float is within
	// UINT32_MAX, so the guarded conversion cannot overflow.
	unsigned quantized = scaled >= 4294967296.0f
			? UINT32_MAX
			: static_cast<unsigned>(scaled);
	quantized >>= delta;

	unsigned bag_offset = quantized > cur_dist_shift
			? quantized - cur_dist_shift
			: 0u;
	bag_offset = hd_min(static_cast<unsigned>(NUM_BAG - 1), bag_offset);
	if ((bag_offset == 0u) && (cur_bag != bag_id)) {
		return (cur_bag + 1u) % NUM_BAG;
	}
