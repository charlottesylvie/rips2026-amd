...
// PathFinder-facing adapter for bf8's outgoing active-frontier Bellman-Ford.
// The graph is immutable and may be shared by independent, stream-affine
// workspaces. Each workspace preserves bf8's single-source kernel sequence and
...
class BellmanFordCsrWorkspace {
 public:
  ...
  // because bf8's protected host iteration loop has no callback hook and must
  // retain its exact launch/synchronization sequence. Multi-source calls run
  // the protected single-source implementation once per stable-deduplicated
  // source. Equal-distance candidates prefer an identity path, then the lower
  // source node id, then the existing packed predecessor-edge ordering.
