...
//
// Add --paths-output bf8.paths.jsonl
...
// CPU adapter. Inputs: PathFinder's outgoing HostCsrF32. Output: bf8's
// outgoing host view with the two fields absent from RIPSCSR1 derived in memory.
// Purpose: let the reusable PathFinder engine use exactly the same device graph
// and protected kernels as the standalone bf8 driver.
HostOutgoingCsrF32 make_outgoing_csr(const HostCsrF32& input) {
  ...
  graph.degree.resize(static_cast<std::size_t>(graph.rows));
  for (Offset row = 0; row < graph.rows; ++row) {
    const Offset begin = graph.rowptr[static_cast<std::size_t>(row)];
    const Offset end = graph.rowptr[static_cast<std::size_t>(row + 1)];
    if (begin < 0 || end < begin || end > graph.nnz) {
  ...
  graph.edge_id.resize(static_cast<std::size_t>(graph.nnz));
  for (Offset edge = 0; edge < graph.nnz; ++edge) {
    graph.edge_id[static_cast<std::size_t>(edge)] = edge;
  }
  ...
  try {
    ...
    while (current != source) {
      ...
      reversed.push_back(
          {from,
           current,
           graph.edge_id[static_cast<std::size_t>(edge)],
  ...
  void ensure_target_capacity(std::size_t target_count) {
    ...
    try {
      ...
      rips_sssp_new::check_hip(
          ...
          "synchronize Bellman-Ford target workspace growth");
    } catch (...) {
...
BellmanFordCsrResult BellmanFordCsrWorkspace::run(
  ...
  if (progress_callback) {
    throw std::invalid_argument(
        ...
        "the protected bf8 iteration loop");
  }
  (void)progress_user_data;
  ...
  std::vector<BellmanFordTargetCandidate> best(targets.size());
  ...
  for (const int source : unique_sources) {
    ...
    const std::vector<unsigned long long> states =
        copy_best_states_to_host(impl_->workspace, graph.rows, stream);
    for (std::size_t target_index = 0;
      ...
      const float distance =
          source == target
              ? 0.0f
              : unpack_state_dist(states[static_cast<std::size_t>(target)]);
      if (!std::isfinite(distance)) {
        continue;
      }

      const std::vector<PathEdge> edges = reconstruct_bellman_ford_adapter_path(
          graph,
          impl_->graph->from_for_edge,
          states,
          source,
          target);
  ...
  for (std::size_t target_index = 0;
    ...
    const BellmanFordTargetCandidate& candidate = best[target_index];
  ...
  // - stopped_on_target means at least one source used bf8's distance bound
