...
void print_usage(const char* program) {
  std::cerr
      ...
      << "  --capacity <int>               Forwarded as a congestion resource capacity.\n"
      << "  --max-pathfinder-iters <int>   Forwarded; enables delta-step congestion routing.\n"
      << "  --route-batch-size <count>     Forwarded congestion batch size.\n"
      << "  --present-factor <float>       Forwarded initial present-cost factor.\n"
      << "  --present-multiplier <float>   Forwarded per-round present-cost multiplier.\n"
      << "  --history-factor <float>       Forwarded historical-cost increment.\n"
      << "  Persistent CSR snapshots are produced by pathfinder_snapshot_benchmark.py.\n"
