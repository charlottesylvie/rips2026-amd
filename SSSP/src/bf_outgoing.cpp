...
//
// Run:
//   ./bf_outgoing data/logicnets_jscl.csrbin data/logicnets_jscl.csrbin.ifmeta.bin paths.jsonl
...
struct Options {
  ...
  std::uint64_t adaptive_bucket_min_candidates = 65536;
  double abs_tolerance = 1e-3;
  double rel_tolerance = 1e-5;
  RelaxBackend relax_backend = RelaxBackend::Direct;
  ...
  if (std::filesystem::exists(outgoing_path)) {
    try {
      ...
      if (outgoing.rows == incoming.rows &&
          outgoing.cols == incoming.cols &&
          outgoing.nnz == incoming.nnz) {
        std::cout << "[bf_outgoing] loaded outgoing CSR: "
...
std::vector<PathEdge> reconstruct_shortest_path_from_pred(
    const HostCsrF32& incoming,
    const std::vector<Offset>& pred_edge,
    int source,
    int target) {
  if (!valid_node(incoming, source) || !valid_node(incoming, target)) {
    throw std::out_of_range("source or target is outside the CSR graph");
  }
  if (pred_edge.size() != static_cast<std::size_t>(incoming.rows)) {
    throw std::invalid_argument("predecessor vector size does not match CSR rows");
  }
  if (source == target) {
    return {};
  }
  ...
  for (Offset guard = 0; guard < incoming.rows && current != source; ++guard) {
    const Offset edge = pred_edge[static_cast<std::size_t>(current)];
...
struct DeviceBucketWorkspace {
  Offset rows = 0;
  Offset candidate_capacity = 0;
  int bucket_count = 0;
  unsigned long long* bucket_counts = nullptr;
  unsigned long long* bucket_offsets = nullptr;
  unsigned long long* bucket_cursors = nullptr;
  Index* candidate_dst = nullptr;
  unsigned long long* candidate_state = nullptr;
  int* merge_marks = nullptr;
  unsigned long long* merged_state = nullptr;
  Index* merged_vertices = nullptr;
  int* merged_count = nullptr;

  DeviceBucketWorkspace() = default;
  DeviceBucketWorkspace(const DeviceBucketWorkspace&) = delete;
  DeviceBucketWorkspace& operator=(const DeviceBucketWorkspace&) = delete;

  DeviceBucketWorkspace(DeviceBucketWorkspace&& other) noexcept {
    *this = std::move(other);
  }

  DeviceBucketWorkspace& operator=(DeviceBucketWorkspace&& other) noexcept {
    if (this != &other) {
      reset();
      rows = other.rows;
      candidate_capacity = other.candidate_capacity;
      bucket_count = other.bucket_count;
      bucket_counts = other.bucket_counts;
      bucket_offsets = other.bucket_offsets;
      bucket_cursors = other.bucket_cursors;
      candidate_dst = other.candidate_dst;
      candidate_state = other.candidate_state;
      merge_marks = other.merge_marks;
      merged_state = other.merged_state;
      merged_vertices = other.merged_vertices;
      merged_count = other.merged_count;
      other.rows = 0;
      other.candidate_capacity = 0;
      other.bucket_count = 0;
      other.bucket_counts = nullptr;
      other.bucket_offsets = nullptr;
      other.bucket_cursors = nullptr;
      other.candidate_dst = nullptr;
      other.candidate_state = nullptr;
      other.merge_marks = nullptr;
      other.merged_state = nullptr;
      other.merged_vertices = nullptr;
      other.merged_count = nullptr;
    }
    return *this;
  }

  ~DeviceBucketWorkspace() {
    reset();
  }

  void reset() {
    if (bucket_counts) {
      (void)hipFree(bucket_counts);
      bucket_counts = nullptr;
    }
    if (bucket_offsets) {
      (void)hipFree(bucket_offsets);
      bucket_offsets = nullptr;
    }
    if (bucket_cursors) {
      (void)hipFree(bucket_cursors);
      bucket_cursors = nullptr;
    }
    if (candidate_dst) {
      (void)hipFree(candidate_dst);
      candidate_dst = nullptr;
    }
    if (candidate_state) {
      (void)hipFree(candidate_state);
      candidate_state = nullptr;
    }
    if (merge_marks) {
      (void)hipFree(merge_marks);
      merge_marks = nullptr;
    }
    if (merged_state) {
      (void)hipFree(merged_state);
      merged_state = nullptr;
    }
    if (merged_vertices) {
      (void)hipFree(merged_vertices);
      merged_vertices = nullptr;
    }
    if (merged_count) {
      (void)hipFree(merged_count);
      merged_count = nullptr;
    }
    rows = 0;
...
DeviceSsspWorkspace make_sssp_workspace(Offset rows, hipStream_t stream) {
  ...
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.next_count), sizeof(int)),
            "hipMalloc SSSP next count");
...
DeviceBucketWorkspace make_bucket_workspace(Offset rows,
                                            Offset candidate_capacity,
                                            int bucket_count,
                                            hipStream_t stream) {
  if (rows <= 0) {
    throw std::runtime_error("bucket workspace expects a nonempty graph");
  }
  if (rows > static_cast<Offset>(std::numeric_limits<Index>::max())) {
    throw std::runtime_error("bucket workspace row count is too large");
  }
  if (candidate_capacity < 0) {
    throw std::runtime_error("bucket workspace candidate capacity is negative");
  }
  if (bucket_count <= 0) {
    throw std::runtime_error("bucket workspace bucket count must be positive");
  }

  DeviceBucketWorkspace workspace;
  workspace.rows = rows;
  workspace.candidate_capacity = candidate_capacity;
  workspace.bucket_count = bucket_count;

  const std::size_t bucket_bytes =
      static_cast<std::size_t>(bucket_count) * sizeof(unsigned long long);
  const std::size_t bucket_offsets_bytes =
      static_cast<std::size_t>(bucket_count + 1) * sizeof(unsigned long long);
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.bucket_counts),
                      bucket_bytes),
            "hipMalloc bucket counts");
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.bucket_offsets),
                      bucket_offsets_bytes),
            "hipMalloc bucket offsets");
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.bucket_cursors),
                      bucket_bytes),
            "hipMalloc bucket cursors");

  if (candidate_capacity > 0) {
    const std::size_t candidate_count =
        static_cast<std::size_t>(candidate_capacity);
    check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.candidate_dst),
                        candidate_count * sizeof(Index)),
              "hipMalloc bucket candidate destinations");
    check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.candidate_state),
                        candidate_count * sizeof(unsigned long long)),
              "hipMalloc bucket candidate states");
  }

  const std::size_t row_count = static_cast<std::size_t>(rows);
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.merge_marks),
                      row_count * sizeof(int)),
            "hipMalloc bucket merge marks");
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.merged_state),
                      row_count * sizeof(unsigned long long)),
            "hipMalloc bucket merged state");
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.merged_vertices),
                      row_count * sizeof(Index)),
            "hipMalloc bucket merged vertices");
  check_hip(hipMalloc(reinterpret_cast<void**>(&workspace.merged_count),
                      sizeof(int)),
            "hipMalloc bucket merged count");
  check_hip(hipMemsetAsync(workspace.merge_marks,
                           0,
                           row_count * sizeof(int),
                           stream),
            "reset bucket merge marks");
  check_hip(hipStreamSynchronize(stream), "synchronize bucket workspace allocation");
  return workspace;
}
...
constexpr int kTinyDegreeUnrollLimit = 12;
constexpr int kBucketBackendDefaultBucketCount = 256;
constexpr int kOutgoingStatusOk = 0;
constexpr int kOutgoingStatusInvalidVertex = 1;
constexpr int kOutgoingStatusInvalidRow = 2;
constexpr int kOutgoingStatusInvalidWeight = 3;
constexpr int kOutgoingStatusFrontierOverflow = 4;
constexpr int kOutgoingStatusInvalidEdge = 5;
constexpr int kOutgoingStatusCandidateOverflow = 6;
constexpr int kOutgoingStatusInvalidBucket = 7;
constexpr int kOutgoingStatusMergeOverflow = 8;
...
int choose_bucket_count(Offset rows) {
  if (rows <= 1) return 1;
  const int row_limited =
      rows < static_cast<Offset>(kBucketBackendDefaultBucketCount)
          ? static_cast<int>(rows)
          : kBucketBackendDefaultBucketCount;
  return std::max(1, row_limited);
}

const char* relax_backend_name(RelaxBackend backend) {
  switch (backend) {
    case RelaxBackend::Direct: return "direct";
    case RelaxBackend::Bucket: return "bucket";
    case RelaxBackend::Adaptive: return "adaptive";
  }
  return "unknown";
}

const char* outgoing_status_description(int status) {
  switch (status) {
    case kOutgoingStatusOk: return "ok";
    case kOutgoingStatusInvalidVertex: return "invalid vertex";
    case kOutgoingStatusInvalidRow: return "invalid outgoing CSR row";
    case kOutgoingStatusInvalidWeight: return "invalid edge weight";
    case kOutgoingStatusFrontierOverflow: return "frontier buffer overflow";
    case kOutgoingStatusInvalidEdge: return "invalid predecessor edge id";
    case kOutgoingStatusCandidateOverflow: return "bucket candidate buffer overflow";
    case kOutgoingStatusInvalidBucket: return "invalid destination bucket";
    case kOutgoingStatusMergeOverflow: return "bucket merge buffer overflow";
    default: return "unknown status";
  }
}

void throw_if_outgoing_status(int status, const char* context) {
  if (status == kOutgoingStatusOk) {
    return;
  }
  throw std::runtime_error(std::string(context) + ": " +
                           outgoing_status_description(status));
}
...
__device__ __forceinline__ bool atomic_relax_packed_state(
...
__device__ __forceinline__ bool atomic_relax_state(unsigned long long* addr,
  ...
  return atomic_relax_packed_state(addr, desired);
}
...
__device__ __forceinline__ int bucket_for_destination(Index dst,
                                                      Offset rows,
                                                      int bucket_count) {
  if (bucket_count <= 1) {
    return 0;
  ...
  int bucket = static_cast<int>(scaled / static_cast<unsigned long long>(rows));
...
__device__ __forceinline__ void relax_outgoing_edge(
  ...
  if (dst < 0 || static_cast<Offset>(dst) >= rows) {
    atomicExch(status, kOutgoingStatusInvalidVertex);
    return;
  }

  if (!finite_device_float(weight) || weight < 0.0f) {
    atomicExch(status, kOutgoingStatusInvalidWeight);
    return;
  }
  ...
  if (original_edge < 0 ||
    ...
    atomicExch(status, kOutgoingStatusInvalidEdge);
    return;
  }
  ...
  if (atomic_relax_state(&best_state[static_cast<Offset>(dst)],
    ...
    if (old_mark != mark_token) {
      ...
      if (slot >= rows) {
        atomicExch(status, kOutgoingStatusFrontierOverflow);
      } else {
...
__global__ void init_outgoing_sssp_kernel(Offset rows,
  ...
  if (row == 0) {
    frontier[0] = source;
    *next_count = 0;
    *status = kOutgoingStatusOk;
  }
}

__global__ void outgoing_frontier_relax_tiny_kernel(
  ...
  if (begin < 0 || deg < 0 || end < begin || end > nnz) {
    atomicExch(status, kOutgoingStatusInvalidRow);
    return;
  }

  if (deg <= kTinyDegreeUnrollLimit) {
#pragma unroll
    for (int edge_offset = 0; edge_offset < kTinyDegreeUnrollLimit; ++edge_offset) {
      if (edge_offset < deg) {
        const Offset edge = begin + static_cast<Offset>(edge_offset);
...
__device__ __forceinline__ bool make_bucket_candidate(
    Index dst,
    float weight,
    Offset original_edge,
    Offset rows,
  ...
  *candidate_state =
      pack_state_float(candidate, static_cast<unsigned int>(original_edge));
  return true;
}

// Experimental SpMSpV-bucket relaxation:
//   sparse frontier = sparse vector x
//   outgoing CSR rows = selected sparse matrix columns/adjacency lists
//   destination row ranges = buckets
//   merged bucket candidates = sparse output y for this Bellman-Ford round
__global__ void bucket_count_candidates_kernel(
    const Offset* __restrict__ rowptr,
    const Index* __restrict__ degree,
    const Index* __restrict__ to,
    const float* __restrict__ values,
    ...
    int bucket_count,
    unsigned long long* __restrict__ bucket_counts,
  ...
  for (Offset edge = begin; edge < end; ++edge) {
    unsigned long long candidate_state = 0;
    const Index dst = to[edge];
    if (!make_bucket_candidate(dst,
                               values[edge],
                               incoming_edge[edge],
                               rows,
                               from_dist,
    ...
    const int bucket = bucket_for_destination(dst, rows, bucket_count);
    if (bucket < 0 || bucket >= bucket_count) {
      atomicExch(status, kOutgoingStatusInvalidBucket);
      continue;
    }
    const unsigned long long old =
        atomicAdd(&bucket_counts[bucket], 1ull);
    if (old >= candidate_capacity) {
      atomicExch(status, kOutgoingStatusCandidateOverflow);
    }
  }
}

__global__ void bucket_scatter_candidates_kernel(
    const Offset* __restrict__ rowptr,
    const Index* __restrict__ degree,
    const Index* __restrict__ to,
    const float* __restrict__ values,
    ...
    const unsigned long long* __restrict__ best_state,
    int bucket_count,
    const unsigned long long* __restrict__ bucket_offsets,
    unsigned long long* __restrict__ bucket_cursors,
    unsigned long long candidate_capacity,
    Index* __restrict__ candidate_dst,
    unsigned long long* __restrict__ candidate_state,
    int* __restrict__ status) {
  const Offset item = logical_thread_id_1d();
  if (item >= static_cast<Offset>(frontier_count)) {
    return;
  ...
  for (Offset edge = begin; edge < end; ++edge) {
    ...
    const unsigned long long slot =
        atomicAdd(&bucket_cursors[bucket], 1ull);
    if (slot >= candidate_capacity || slot >= bucket_offsets[bucket + 1]) {
      atomicExch(status, kOutgoingStatusCandidateOverflow);
      continue;
    }
    candidate_dst[slot] = dst;
    candidate_state[slot] = state;
  }
}

__global__ void bucket_init_touched_vertices_kernel(
    const Index* __restrict__ candidate_dst,
    unsigned long long candidate_count,
    Offset rows,
    int mark_token,
    int* __restrict__ merge_marks,
    unsigned long long* __restrict__ merged_state,
    Index* __restrict__ merged_vertices,
    int* __restrict__ merged_count,
    int* __restrict__ status) {
  const unsigned long long item =
      static_cast<unsigned long long>(logical_thread_id_1d());
  if (item >= candidate_count) {
    return;
  }

  const Index dst = candidate_dst[item];
  if (dst < 0 || static_cast<Offset>(dst) >= rows) {
    atomicExch(status, kOutgoingStatusInvalidVertex);
    return;
  }

  const int old_mark = atomicExch(&merge_marks[static_cast<Offset>(dst)],
                                  mark_token);
  if (old_mark != mark_token) {
    merged_state[static_cast<Offset>(dst)] =
        pack_state_bits(kPackedInfinityBits, kPackedNoPredEdge);
    const int slot = atomicAdd(merged_count, 1);
    if (slot >= rows) {
      atomicExch(status, kOutgoingStatusMergeOverflow);
    } else {
      merged_vertices[slot] = dst;
    }
  }
}

__global__ void bucket_merge_candidates_kernel(
    const Index* __restrict__ candidate_dst,
    const unsigned long long* __restrict__ candidate_state,
    unsigned long long candidate_count,
    Offset rows,
    unsigned long long* __restrict__ merged_state,
    int* __restrict__ status) {
  ...
  atomic_relax_packed_state(&merged_state[static_cast<Offset>(dst)],
                            candidate_state[item]);
}

__global__ void bucket_apply_merged_candidates_kernel(
    const Index* __restrict__ merged_vertices,
    int merged_count,
    Offset rows,
    int mark_token,
    const unsigned long long* __restrict__ merged_state,
    unsigned long long* __restrict__ best_state,
    Index* __restrict__ next_frontier,
    int* __restrict__ next_marks,
    int* __restrict__ next_count,
    int* __restrict__ status) {
  const Offset item = logical_thread_id_1d();
  if (item >= static_cast<Offset>(merged_count)) {
    return;
  }

  const Index dst = merged_vertices[item];
  if (dst < 0 || static_cast<Offset>(dst) >= rows) {
    atomicExch(status, kOutgoingStatusInvalidVertex);
    return;
  }
  ...
  const unsigned long long candidate =
      merged_state[static_cast<Offset>(dst)];
  if (atomic_relax_packed_state(&best_state[static_cast<Offset>(dst)],
                                candidate)) {
    const int old_mark = atomicExch(&next_marks[static_cast<Offset>(dst)],
                                    mark_token);
    if (old_mark != mark_token) {
...
struct OutgoingSsspResult {
  ...
  std::vector<Offset> pred_edge;
  int iterations_used = 0;
  int direct_iterations = 0;
  int bucket_iterations = 0;
  unsigned long long estimated_candidate_edges = 0;
  bool converged = false;
};

void copy_outgoing_sssp_result(OutgoingSsspResult* result,
                               Offset rows,
                               int source,
                               DeviceSsspWorkspace& workspace,
                               hipStream_t stream) {
  result->dist.resize(static_cast<std::size_t>(rows));
  result->pred_edge.resize(static_cast<std::size_t>(rows));
  std::vector<unsigned long long> best_state(static_cast<std::size_t>(rows));
  check_hip(hipMemcpyAsync(best_state.data(),
                           workspace.best_state,
                           best_state.size() * sizeof(unsigned long long),
                           hipMemcpyDeviceToHost,
                           stream),
            "copy outgoing SSSP best states");
  check_hip(hipStreamSynchronize(stream), "synchronize outgoing SSSP result copy");

  for (std::size_t row = 0; row < best_state.size(); ++row) {
    const unsigned int dist_bits =
        static_cast<unsigned int>(best_state[row] >> 32);
    const unsigned int edge_bits =
        static_cast<unsigned int>(best_state[row] & kPackedNoPredEdge);
    result->dist[row] = host_float_from_bits(dist_bits);
    result->pred_edge[row] =
        (static_cast<Offset>(row) == static_cast<Offset>(source) ||
         edge_bits == kPackedNoPredEdge)
            ? -1
            : static_cast<Offset>(edge_bits);
  }
}

int run_direct_outgoing_iteration(const DeviceOutgoingCsrF32& graph,
                                  DeviceSsspWorkspace& workspace,
                                  int frontier_count,
                                  int mark_token,
                                  hipStream_t stream) {
  ...
  check_hip(hipMemsetAsync(workspace.next_count, 0, sizeof(int), stream),
            "reset outgoing SSSP next count");
  check_hip(hipMemsetAsync(workspace.status, 0, sizeof(int), stream),
            "reset outgoing SSSP status");

  hipLaunchKernelGGL(outgoing_frontier_relax_tiny_kernel,
                     ...
                     workspace.next_count,
                     workspace.status);
  check_hip(hipGetLastError(), "launch outgoing SSSP relaxation");

  check_hip(hipMemcpyAsync(&next_frontier_count,
                           workspace.next_count,
                           sizeof(int),
                           hipMemcpyDeviceToHost,
                           stream),
            "copy outgoing SSSP frontier count");
  ...
  throw_if_outgoing_status(h_status, "outgoing SSSP relaxation failed");

  std::swap(workspace.frontier, workspace.next_frontier);
  return next_frontier_count;
}

int run_bucketed_outgoing_iteration(
    const DeviceOutgoingCsrF32& graph,
    DeviceSsspWorkspace& workspace,
    DeviceBucketWorkspace& bucket_workspace,
    int frontier_count,
    int mark_token,
    std::vector<unsigned long long>& h_bucket_counts,
    std::vector<unsigned long long>& h_bucket_offsets,
    unsigned long long* candidate_count_out,
    hipStream_t stream) {
  constexpr int threads = kFrontierBlockSize;
  const int bucket_count = bucket_workspace.bucket_count;
  int next_frontier_count = 0;
  int h_status = 0;
  ...
  check_hip(hipMemsetAsync(bucket_workspace.bucket_counts,
                           0,
            ...
            "reset bucket counts");
  check_hip(hipMemsetAsync(workspace.next_count, 0, sizeof(int), stream),
            "reset bucketed outgoing SSSP next count");
  check_hip(hipMemsetAsync(bucket_workspace.merged_count, 0, sizeof(int), stream),
            "reset bucket merged count");
  check_hip(hipMemsetAsync(workspace.status, 0, sizeof(int), stream),
            "reset bucketed outgoing SSSP status");

  hipLaunchKernelGGL(bucket_count_candidates_kernel,
                     grid_for_items(frontier_count, threads),
                     dim3(threads),
                     0,
                     stream,
                     ...
                     workspace.best_state,
                     bucket_count,
                     bucket_workspace.bucket_counts,
                     static_cast<unsigned long long>(
                         bucket_workspace.candidate_capacity),
                     workspace.status);
  check_hip(hipGetLastError(), "launch bucket candidate counting");

  check_hip(hipMemcpyAsync(h_bucket_counts.data(),
                           bucket_workspace.bucket_counts,
                           h_bucket_counts.size() * sizeof(unsigned long long),
                           hipMemcpyDeviceToHost,
                           stream),
            "copy bucket counts");
  check_hip(hipMemcpyAsync(&h_status,
                           ...
                           hipMemcpyDeviceToHost,
                           stream),
            "copy bucket count status");
  check_hip(hipStreamSynchronize(stream), "synchronize bucket counts");
  throw_if_outgoing_status(h_status, "bucket candidate counting failed");

  h_bucket_offsets[0] = 0;
  for (int bucket = 0; bucket < bucket_count; ++bucket) {
    h_bucket_offsets[static_cast<std::size_t>(bucket + 1)] =
        h_bucket_offsets[static_cast<std::size_t>(bucket)] +
        h_bucket_counts[static_cast<std::size_t>(bucket)];
  }
  const unsigned long long candidate_count =
      h_bucket_offsets[static_cast<std::size_t>(bucket_count)];
  if (candidate_count_out) {
    *candidate_count_out = candidate_count;
  }
  if (candidate_count >
      static_cast<unsigned long long>(bucket_workspace.candidate_capacity)) {
    throw std::runtime_error("bucket candidate count exceeds workspace capacity");
  }

  if (candidate_count > 0) {
    check_hip(hipMemcpyAsync(bucket_workspace.bucket_offsets,
                             h_bucket_offsets.data(),
                             h_bucket_offsets.size() *
                                 sizeof(unsigned long long),
                             hipMemcpyHostToDevice,
                             stream),
              "copy bucket offsets");
    check_hip(hipMemcpyAsync(bucket_workspace.bucket_cursors,
                             bucket_workspace.bucket_offsets,
                             static_cast<std::size_t>(bucket_count) *
                                 sizeof(unsigned long long),
                             hipMemcpyDeviceToDevice,
                             stream),
              "initialize bucket cursors");

    hipLaunchKernelGGL(bucket_scatter_candidates_kernel,
                       grid_for_items(frontier_count, threads),
                       dim3(threads),
                       0,
                       stream,
                       ...
                       bucket_count,
                       bucket_workspace.bucket_offsets,
                       bucket_workspace.bucket_cursors,
                       static_cast<unsigned long long>(
                           bucket_workspace.candidate_capacity),
                       bucket_workspace.candidate_dst,
                       bucket_workspace.candidate_state,
                       workspace.status);
    check_hip(hipGetLastError(), "launch bucket candidate scatter");

    hipLaunchKernelGGL(bucket_init_touched_vertices_kernel,
                       grid_for_items(static_cast<Offset>(candidate_count),
                                      threads),
                       dim3(threads),
                       0,
                       stream,
                       ...
                       graph.rows,
                       mark_token,
                       bucket_workspace.merge_marks,
                       bucket_workspace.merged_state,
                       bucket_workspace.merged_vertices,
                       bucket_workspace.merged_count,
                       workspace.status);
    check_hip(hipGetLastError(), "launch bucket touched vertex initialization");

    hipLaunchKernelGGL(bucket_merge_candidates_kernel,
                       grid_for_items(static_cast<Offset>(candidate_count),
                                      threads),
                       dim3(threads),
                       0,
                       ...
                       bucket_workspace.merged_state,
                       workspace.status);
    check_hip(hipGetLastError(), "launch bucket candidate merge");

    int h_merged_count = 0;
    check_hip(hipMemcpyAsync(&h_merged_count,
                             bucket_workspace.merged_count,
                             sizeof(int),
                             hipMemcpyDeviceToHost,
                             stream),
              "copy bucket merged count");
    check_hip(hipMemcpyAsync(&h_status,
                             workspace.status,
                             sizeof(int),
                             hipMemcpyDeviceToHost,
                             stream),
              "copy bucket merge status");
    check_hip(hipStreamSynchronize(stream), "synchronize bucket merge");
    throw_if_outgoing_status(h_status, "bucket candidate merge failed");
    if (h_merged_count < 0 ||
        static_cast<Offset>(h_merged_count) > graph.rows) {
      throw std::runtime_error("bucket merged count is out of range");
    }

    if (h_merged_count > 0) {
      hipLaunchKernelGGL(bucket_apply_merged_candidates_kernel,
                         grid_for_items(h_merged_count, threads),
                         dim3(threads),
                         0,
                         stream,
                         bucket_workspace.merged_vertices,
                         ...
                         workspace.next_count,
                         workspace.status);
      check_hip(hipGetLastError(), "launch bucket merged candidate apply");
    }
  }
  ...
  check_hip(hipMemcpyAsync(&next_frontier_count,
                           ...
                           hipMemcpyDeviceToHost,
                           stream),
            "copy bucketed outgoing SSSP frontier count");
  check_hip(hipMemcpyAsync(&h_status,
                           workspace.status,
                           sizeof(int),
                           hipMemcpyDeviceToHost,
                           stream),
            "copy bucketed outgoing SSSP status");
  check_hip(hipStreamSynchronize(stream),
            "synchronize bucketed outgoing SSSP iteration");
  throw_if_outgoing_status(h_status, "bucketed outgoing SSSP relaxation failed");
...
OutgoingSsspResult run_outgoing_frontier_sssp(const DeviceOutgoingCsrF32& graph,
  ...
  for (int iter = 0; iter < max_iters && frontier_count > 0; ++iter) {
    frontier_count = run_direct_outgoing_iteration(graph,
                                                   workspace,
                                                   frontier_count,
                                                   iter + 1,
                                                   stream);
    ++result.direct_iterations;
    result.iterations_used = iter + 1;
  }

  result.converged = frontier_count == 0;
  copy_outgoing_sssp_result(&result, graph.rows, source, workspace, stream);
  return result;
}

OutgoingSsspResult run_bucketed_outgoing_frontier_sssp(
    const DeviceOutgoingCsrF32& graph,
    DeviceSsspWorkspace& workspace,
    DeviceBucketWorkspace& bucket_workspace,
    int source,
    ...
    hipStream_t stream) {
  if (graph.rows <= 0 || graph.rows != graph.cols) {
    throw std::runtime_error("bucketed outgoing SSSP expects a nonempty square graph");
  }
  if (workspace.rows != graph.rows) {
    throw std::runtime_error("SSSP workspace row count does not match graph");
  }
  if (bucket_workspace.rows != graph.rows) {
    throw std::runtime_error("bucket workspace row count does not match graph");
  }
  if (bucket_workspace.candidate_capacity < graph.nnz) {
    throw std::runtime_error("bucket workspace candidate capacity is too small");
  }
  if (bucket_workspace.bucket_count <= 0) {
    throw std::runtime_error("bucket workspace has no buckets");
  }
  if (source < 0 || static_cast<Offset>(source) >= graph.rows) {
    throw std::runtime_error("source is outside outgoing graph");
  }
  ...
  hipLaunchKernelGGL(init_outgoing_sssp_kernel,
                     ...
                     workspace.next_count,
                     workspace.status);
  check_hip(hipGetLastError(), "launch bucketed outgoing SSSP initialization");
  check_hip(hipMemsetAsync(bucket_workspace.merge_marks,
                           0,
                           static_cast<std::size_t>(graph.rows) * sizeof(int),
                           stream),
            "reset bucket merge marks");
  check_hip(hipStreamSynchronize(stream),
            "synchronize bucketed outgoing SSSP initialization");
  ...
  std::vector<unsigned long long> h_bucket_counts;
  std::vector<unsigned long long> h_bucket_offsets;
  h_bucket_counts.assign(static_cast<std::size_t>(bucket_count), 0);
  h_bucket_offsets.assign(static_cast<std::size_t>(bucket_count + 1), 0);

  for (int iter = 0; iter < max_iters && frontier_count > 0; ++iter) {
    unsigned long long candidate_count = 0;
    frontier_count = run_bucketed_outgoing_iteration(graph,
                                                     workspace,
                                                     bucket_workspace,
                                                     frontier_count,
                                                     iter + 1,
                                                     ...
                                                     &candidate_count,
                                                     stream);
    ++result.bucket_iterations;
    result.estimated_candidate_edges += candidate_count;
    result.iterations_used = iter + 1;
  }
...
unsigned long long estimate_frontier_candidate_edges(const DeviceOutgoingCsrF32& graph,
                                                     int frontier_count) {
  if (frontier_count <= 0 || graph.rows <= 0 || graph.nnz <= 0) {
    return 0;
  }
  const long double average_degree =
      static_cast<long double>(graph.nnz) / static_cast<long double>(graph.rows);
  const long double estimate =
      average_degree * static_cast<long double>(frontier_count);
  ...
  return static_cast<unsigned long long>(estimate + 0.5L);
}

OutgoingSsspResult run_adaptive_outgoing_frontier_sssp(
    const DeviceOutgoingCsrF32& graph,
    DeviceSsspWorkspace& workspace,
    DeviceBucketWorkspace& bucket_workspace,
    int source,
    int max_iters,
    std::uint64_t bucket_min_candidates,
    hipStream_t stream) {
  if (graph.rows <= 0 || graph.rows != graph.cols) {
    throw std::runtime_error("adaptive outgoing SSSP expects a nonempty square graph");
  }
  if (workspace.rows != graph.rows) {
    throw std::runtime_error("SSSP workspace row count does not match graph");
  }
  if (bucket_workspace.rows != 0 && bucket_workspace.rows != graph.rows) {
    throw std::runtime_error("bucket workspace row count does not match graph");
  }
  if (bucket_workspace.rows != 0 &&
      bucket_workspace.candidate_capacity < graph.nnz) {
    throw std::runtime_error("bucket workspace candidate capacity is too small");
  }
  if (bucket_workspace.rows != 0 && bucket_workspace.bucket_count <= 0) {
    throw std::runtime_error("bucket workspace has no buckets");
  }
  if (source < 0 || static_cast<Offset>(source) >= graph.rows) {
    throw std::runtime_error("source is outside outgoing graph");
  ...
  hipLaunchKernelGGL(init_outgoing_sssp_kernel,
                     ...
                     workspace.next_count,
                     workspace.status);
  check_hip(hipGetLastError(), "launch adaptive outgoing SSSP initialization");
  if (bucket_workspace.rows != 0) {
    check_hip(hipMemsetAsync(bucket_workspace.merge_marks,
                             0,
                             static_cast<std::size_t>(graph.rows) * sizeof(int),
                             stream),
              "reset adaptive bucket merge marks");
  }
  check_hip(hipStreamSynchronize(stream),
            "synchronize adaptive outgoing SSSP initialization");
  ...
  for (int iter = 0; iter < max_iters && frontier_count > 0; ++iter) {
    const unsigned long long estimated_candidates =
        estimate_frontier_candidate_edges(graph, frontier_count);
    const bool use_bucket =
        estimated_candidates >= bucket_min_candidates &&
        graph.nnz > 0;

    if (use_bucket) {
      if (bucket_workspace.rows == 0) {
        const int bucket_count = choose_bucket_count(graph.rows);
        std::cout << "[bf_outgoing] adaptive allocating bucket relaxation workspace"
                  << " buckets=" << bucket_count
                  << " candidate_capacity=" << graph.nnz << '\n';
        bucket_workspace =
            make_bucket_workspace(graph.rows, graph.nnz, bucket_count, stream);
      }
      const int bucket_count = bucket_workspace.bucket_count;
      if (h_bucket_counts.size() != static_cast<std::size_t>(bucket_count)) {
        h_bucket_counts.assign(static_cast<std::size_t>(bucket_count), 0);
        h_bucket_offsets.assign(static_cast<std::size_t>(bucket_count + 1), 0);
      }
      unsigned long long candidate_count = 0;
      ...
      frontier_count = run_direct_outgoing_iteration(graph,
                                                     ...
                                                     stream);
      ++result.direct_iterations;
      result.estimated_candidate_edges += estimated_candidates;
    }
    result.iterations_used = iter + 1;
  }
...
RelaxBackend parse_relax_backend(const std::string& text) {
  if (text == "direct") {
    return RelaxBackend::Direct;
  }
  if (text == "bucket") {
    return RelaxBackend::Bucket;
  }
  if (text == "adaptive") {
    return RelaxBackend::Adaptive;
  }
  throw std::runtime_error("--relax-backend must be direct, bucket, or adaptive");
}
...
void print_usage(const char* program) {
  std::cerr
      ...
      << "  --relax-backend <name>    direct, bucket, or adaptive. Default: direct.\n"
      << "  --adaptive-bucket-min-candidates <n>\n"
      << "                              Adaptive mode uses bucket at or above n estimated candidates. Default: 65536.\n"
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--max-iters") {
      ...
      options.rel_tolerance = parse_double(require_value(&i, argc, argv, "--rel-tol"),
                                           "--rel-tol");
    } else if (arg == "--relax-backend") {
      options.relax_backend =
          parse_relax_backend(require_value(&i, argc, argv, "--relax-backend"));
    } else if (arg == "--adaptive-bucket-min-candidates") {
      options.adaptive_bucket_min_candidates = static_cast<std::uint64_t>(
          parse_size(require_value(&i,
                                   argc,
                                   argv,
                                   "--adaptive-bucket-min-candidates"),
                     "--adaptive-bucket-min-candidates"));
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "[bf_outgoing] relax_backend="
              << relax_backend_name(options.relax_backend) << '\n';

    std::cout << "[bf_outgoing] loading CSR: " << csr_path.string() << '\n';
    HostCsrF32 graph = load_csrbin(csr_path);
    std::cout << "[bf_outgoing] graph rows=" << graph.rows
              << " nnz=" << graph.nnz
              << " orientation=incoming(row v, col u means u -> v)\n";

    OutgoingCsrF32 outgoing_graph = load_or_create_outgoing_csrbin(graph, csr_path);
    std::cout << "[bf_outgoing] outgoing graph rows=" << outgoing_graph.rows
              << " nnz=" << outgoing_graph.nnz
              << " orientation=outgoing(row u stores edges u -> v)\n";

    std::cout << "[bf_outgoing] loading metadata: " << metadata_path.string() << '\n';
    ...
    std::cout << "[bf_outgoing] route_requests="
              << metadata.route_requests.size() << '\n';
    ...
    std::cout << "[bf_outgoing] unique_sources=" << work.size()
    ...
    try {
      std::cout << "[bf_outgoing] copying outgoing CSR to GPU once\n";
      DeviceOutgoingCsrOwner d_graph =
          copy_outgoing_csr_to_device(outgoing_graph, stream);
      DeviceSsspWorkspace sssp_workspace =
          make_sssp_workspace(outgoing_graph.rows, stream);
      DeviceBucketWorkspace bucket_workspace;
      if (options.relax_backend == RelaxBackend::Bucket) {
        const int bucket_count = choose_bucket_count(outgoing_graph.rows);
        // Conservative first version: a unique frontier can generate at most
        // graph.nnz outgoing candidates, so this avoids per-iteration reallocs.
        std::cout << "[bf_outgoing] allocating bucket relaxation workspace"
                  << " buckets=" << bucket_count
                  << " candidate_capacity=" << outgoing_graph.nnz << '\n';
        bucket_workspace =
            make_bucket_workspace(outgoing_graph.rows,
                                  outgoing_graph.nnz,
                                  bucket_count,
                                  stream);
      } else if (options.relax_backend == RelaxBackend::Adaptive) {
        std::cout << "[bf_outgoing] adaptive bucket min estimated candidates="
                  << options.adaptive_bucket_min_candidates << '\n';
      }
      ...
      for (std::size_t source_index = 0; source_index < source_count; ++source_index) {
        ...
        OutgoingSsspResult result;
        if (options.relax_backend == RelaxBackend::Bucket) {
          result = run_bucketed_outgoing_frontier_sssp(d_graph.view,
                                                       sssp_workspace,
                                                       bucket_workspace,
                                                       source_work.source,
                                                       options.max_iters,
          ...
          result = run_adaptive_outgoing_frontier_sssp(
              d_graph.view,
              sssp_workspace,
              bucket_workspace,
              source_work.source,
          ...
          result = run_outgoing_frontier_sssp(d_graph.view,
                                             sssp_workspace,
                                             source_work.source,
                                             options.max_iters,
                                             stream);
        ...
        for (const Query& query : source_work.queries) {
          ...
          std::vector<PathEdge> edges =
              reconstruct_shortest_path_from_pred(graph,
                                                  result.pred_edge,
                                                  query.source,
                                                  query.target);
        ...
        if (should_print_summary) {
          ...
          std::cout << "[bf_outgoing] source " << completed
                    << "/" << source_count
                    << " node=" << source_work.source
                    << " queries=" << source_work.queries.size()
	                    << " iterations_used=" << result.iterations_used
	                    << " converged=" << (result.converged ? "yes" : "no")
	                    << " direct_iters=" << result.direct_iterations
	                    << " bucket_iters=" << result.bucket_iterations
	                    << " candidate_work="
	                    << result.estimated_candidate_edges
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf_outgoing] warning: hipStreamDestroy during cleanup failed: "
    ...
    std::cout << "[bf_outgoing] wrote " << paths_written
    ...
    std::cerr << "[bf_outgoing] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
