...
//     SSSP/src/bf_outgoing_copy.cpp \
//     -o bf_outgoing_copy
//
// Run:
//   ./bf_outgoing_copy data/logicnets_jscl.csrbin \
  ...
  if (std::filesystem::exists(outgoing_path)) {
    try {
      ...
      if (outgoing.rows == incoming.rows &&
          outgoing.cols == incoming.cols &&
          outgoing.nnz == incoming.nnz) {
        std::cout << "[bf_outgoing_copy] loaded outgoing CSR: "
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "[bf_outgoing_copy] loading CSR: " << csr_path.string() << '\n';
    HostCsrF32 graph = load_csrbin(csr_path);
    std::cout << "[bf_outgoing_copy] graph rows=" << graph.rows
              << " nnz=" << graph.nnz
              << " orientation=incoming(row v, col u means u -> v)\n";

    OutgoingCsrF32 outgoing_graph = load_or_create_outgoing_csrbin(graph, csr_path);
    std::cout << "[bf_outgoing_copy] outgoing graph rows=" << outgoing_graph.rows
              << " nnz=" << outgoing_graph.nnz
              << " orientation=outgoing(row u stores edges u -> v)\n";

    std::cout << "[bf_outgoing_copy] loading metadata: " << metadata_path.string() << '\n';
    ...
    std::cout << "[bf_outgoing_copy] route_requests="
              << metadata.route_requests.size() << '\n';
    ...
    std::cout << "[bf_outgoing_copy] unique_sources=" << work.size()
    ...
    try {
      std::cout << "[bf_outgoing_copy] copying outgoing CSR to GPU once\n";
      DeviceOutgoingCsrOwner d_graph =
          copy_outgoing_csr_to_device(outgoing_graph, stream);
      DeviceSsspWorkspace sssp_workspace =
          make_sssp_workspace(outgoing_graph.rows, gather_capacity, stream);
      ...
      for (std::size_t source_index = 0; source_index < source_count; ++source_index) {
        ...
        if (should_print_summary) {
          ...
          std::cout << "[bf_outgoing_copy] source " << completed
                    << "/" << source_count
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf_outgoing_copy] warning: hipStreamDestroy during cleanup failed: "
    ...
    std::cout << "[bf_outgoing_copy] wrote " << paths_written
    ...
    std::cerr << "[bf_outgoing_copy] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
