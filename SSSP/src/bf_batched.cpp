// Batched HIP Bellman-Ford for validator JSONL generation.
//
// This driver reads a RIPS .csrbin graph and .csrbin.ifmeta.bin sidecar,
// groups metadata route queries by unique source node, and runs Bellman-Ford
// over batches of sources. The batched distance object is an N x B sparse CSR
// matrix, where B is --batch-size. Each relaxation reuses the same graph CSR:
//
...
struct BatchResult {
  std::vector<float> dense_dist;  // row-major: dist[row * batch_cols + col]
...
__global__ void init_batched_source_rowptr_kernel(Offset rows,
                                                  int batch_cols,
                                                  const int* source_nodes,
                                                  Offset* rowptr) {
  const Offset i = logical_thread_id_1d();
  if (i > rows) return;

  Offset count = 0;
  for (int col = 0; col < batch_cols; ++col) {
    if (static_cast<Offset>(source_nodes[col]) < i) {
      ++count;
    }
  }
  rowptr[i] = count;
...
__global__ void write_batched_source_entries_kernel(int batch_cols,
                                                    const int* source_nodes,
                                                    const int* source_cols,
                                                    const Offset* rowptr,
                                                    Index* colind,
                                                    float* values) {
  const int i = static_cast<int>(logical_thread_id_1d());
  if (i >= batch_cols) return;

  const Offset row = static_cast<Offset>(source_nodes[i]);
  const Offset dst = rowptr[row];
  colind[dst] = source_cols[i];
  values[dst] = 0.0f;
}
...
DeviceCsrOwner make_device_batched_source_matrix(Offset rows,
                                                 const std::vector<int>& sources,
                                                 hipStream_t stream) {
  if (sources.empty() || sources.size() > static_cast<std::size_t>(kMaxBatchSize)) {
    throw std::runtime_error("invalid source batch size");
  }

  std::vector<std::pair<int, int>> sorted;
  sorted.reserve(sources.size());
  for (std::size_t col = 0; col < sources.size(); ++col) {
    sorted.emplace_back(sources[col], static_cast<int>(col));
  }
  std::sort(sorted.begin(), sorted.end());

  std::vector<int> sorted_nodes(sorted.size());
  std::vector<int> sorted_cols(sorted.size());
  for (std::size_t i = 0; i < sorted.size(); ++i) {
    sorted_nodes[i] = sorted[i].first;
    sorted_cols[i] = sorted[i].second;
  }

  DeviceCsrOwner matrix;
  int* d_source_nodes = nullptr;
  int* d_source_cols = nullptr;
  auto cleanup_temp = [&]() {
    if (d_source_nodes) (void)hipFree(d_source_nodes);
    if (d_source_cols) (void)hipFree(d_source_cols);
    d_source_nodes = nullptr;
    d_source_cols = nullptr;
  };

  try {
    const Offset batch_cols = static_cast<Offset>(sources.size());
    check_hip(hipMalloc(reinterpret_cast<void**>(&matrix.rowptr),
                        static_cast<std::size_t>(rows + 1) * sizeof(Offset)),
              "hipMalloc batched source rowptr");
    check_hip(hipMalloc(reinterpret_cast<void**>(&matrix.colind),
                        sources.size() * sizeof(Index)),
              "hipMalloc batched source colind");
    check_hip(hipMalloc(reinterpret_cast<void**>(&matrix.values),
                        sources.size() * sizeof(float)),
              "hipMalloc batched source values");
    check_hip(hipMalloc(reinterpret_cast<void**>(&d_source_nodes),
                        sources.size() * sizeof(int)),
              "hipMalloc source nodes");
    check_hip(hipMalloc(reinterpret_cast<void**>(&d_source_cols),
                        sources.size() * sizeof(int)),
              "hipMalloc source cols");
    check_hip(hipMemcpyAsync(d_source_nodes,
                             sorted_nodes.data(),
                             sorted_nodes.size() * sizeof(int),
                             hipMemcpyHostToDevice,
                             stream),
              "copy source nodes");
    check_hip(hipMemcpyAsync(d_source_cols,
                             sorted_cols.data(),
                             sorted_cols.size() * sizeof(int),
                             hipMemcpyHostToDevice,
                             stream),
              "copy source cols");

    constexpr int threads = 256;
    hipLaunchKernelGGL(init_batched_source_rowptr_kernel,
                       grid_for_items(rows + 1, threads),
                       dim3(threads),
                       0,
                       stream,
                       rows,
                       static_cast<int>(batch_cols),
                       d_source_nodes,
                       matrix.rowptr);
    check_hip(hipGetLastError(), "launch batched source rowptr init kernel");

    hipLaunchKernelGGL(write_batched_source_entries_kernel,
                       grid_for_items(batch_cols, threads),
                       dim3(threads),
                       0,
                       stream,
                       static_cast<int>(batch_cols),
                       ...
                       matrix.colind,
                       matrix.values);
    check_hip(hipGetLastError(), "launch batched source entry write kernel");
    check_hip(hipStreamSynchronize(stream), "synchronize batched source init");
    cleanup_temp();

    matrix.view.rows = rows;
    matrix.view.cols = batch_cols;
    matrix.view.nnz = batch_cols;
...
DeviceCsrOwner merge_batched_distances_on_device(const minplus_sparse::DeviceCsrF32& old_dist,
  ...
  if (old_dist.cols <= 0 || old_dist.cols > kMaxBatchSize) {
    throw std::runtime_error("batched merge supports batch sizes in [1, 64]");
  }
...
void print_bf_iteration_progress(std::size_t batch_index,
  ...
  std::cout << '\r'
            << "[bf_batched] batch " << batch_index << "/" << batch_count << ' '
...
BatchResult bellman_ford_batched(const minplus_sparse::DeviceCsrF32& d_graph,
                                 Offset rows,
                                 const std::vector<int>& sources,
                                 int max_iters,
                                 int progress_every,
  ...
  DeviceCsrOwner d_dist = make_device_batched_source_matrix(rows, sources, stream);
  BatchResult result;
  ...
  for (int iter = 0; iter < max_iters; ++iter) {
    ...
    if (should_print) {
      ...
      print_bf_iteration_progress(batch_index,
                                  batch_count,
...
float dense_dist_at(const std::vector<float>& dense,
                    ...
                    int node) {
  (void)rows;
  return dense[static_cast<std::size_t>(node) * static_cast<std::size_t>(batch_cols) +
               static_cast<std::size_t>(batch_col)];
}
...
std::vector<PathEdge> reconstruct_shortest_path(const HostCsrF32& graph,
  ...
  if (!std::isfinite(dense_dist_at(dense_dist, graph.rows, batch_cols, batch_col, target))) {
    return {};
  }
  ...
  for (Offset guard = 0; guard < graph.rows && current != source; ++guard) {
    ...
    const float current_dist =
        dense_dist_at(dense_dist, graph.rows, batch_cols, batch_col, current);
    Offset best_edge = -1;
    int best_pred = -1;
    ...
    for (Offset edge = graph.rowptr[row]; edge < graph.rowptr[row + 1]; ++edge) {
      ...
      const float pred_dist =
          dense_dist_at(dense_dist, graph.rows, batch_cols, batch_col, pred);
...
void print_usage(const char* program) {
  std::cerr
      ...
      << "Options:\n"
      << "  --batch-size <n>         Sources per batched BF run. Default: 4, max: 64.\n"
      ...
      << "  --no-bf-progress         Disable the per-batch BF progress bar.\n"
...
Options parse_options(int argc,
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--batch-size") {
      options.batch_size =
          parse_int(require_value(&i, argc, argv, "--batch-size"), "--batch-size");
  ...
  if (options.batch_size <= 0 || options.batch_size > kMaxBatchSize) {
    throw std::runtime_error("--batch-size must be in [1, 64]");
  }
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "[bf_batched] loading CSR: " << csr_path.string() << '\n';
    HostCsrF32 graph = load_csrbin(csr_path);
    std::cout << "[bf_batched] graph rows=" << graph.rows
              << " nnz=" << graph.nnz
              << " orientation=incoming(row v, col u means u -> v)\n";

    std::cout << "[bf_batched] loading metadata: " << metadata_path.string() << '\n';
    ...
    const std::size_t batch_count =
        (source_count + static_cast<std::size_t>(options.batch_size) - 1) /
        static_cast<std::size_t>(options.batch_size);
    std::cout << "[bf_batched] route_requests=" << metadata.route_requests.size()
              ...
              << " batches=" << batch_count << '\n';
    if (source_count != 0) {
      const std::size_t full_batch_cols =
          std::min(source_count, static_cast<std::size_t>(options.batch_size));
      const double dense_mib =
          static_cast<double>(graph.rows) * static_cast<double>(full_batch_cols) *
          static_cast<double>(sizeof(float)) / (1024.0 * 1024.0);
      std::cout << "[bf_batched] dense distance copy per full batch ~= "
                << std::fixed << std::setprecision(2) << dense_mib
                << " MiB (N * batch_size * 4 bytes)\n";
    }
    ...
    try {
      std::cout << "[bf_batched] copying CSR to GPU once\n";
      DeviceCsrOwner d_graph = bf_csr_detail::copy_host_csr_to_device(graph, stream);

      for (std::size_t batch_begin = 0, batch_index = 1;
           batch_begin < source_count;
           batch_begin += static_cast<std::size_t>(options.batch_size), ++batch_index) {
        const std::size_t batch_end =
            std::min(source_count, batch_begin + static_cast<std::size_t>(options.batch_size));
        std::vector<int> batch_sources;
        batch_sources.reserve(batch_end - batch_begin);
        for (std::size_t i = batch_begin; i < batch_end; ++i) {
          batch_sources.push_back(work[i].source);
        }

        const auto batch_start_time = std::chrono::steady_clock::now();
        std::cout << "[bf_batched] running batch " << batch_index << "/" << batch_count
                  << " cols=" << batch_sources.size()
                  << " first_source=" << batch_sources.front() << '\n'
                  << std::flush;

        BatchResult result = bellman_ford_batched(d_graph.view,
                                                  graph.rows,
                                                  batch_sources,
                                                  options.max_iters,
                                                  options.bf_progress_every,
        ...
        std::cout << "[bf_batched] batch " << batch_index
                  << " iterations_used=" << result.iterations_used
                  << " converged=" << (result.converged ? "yes" : "no")
                  << "; reconstructing paths\n";

        for (std::size_t local_col = 0; local_col < batch_sources.size(); ++local_col) {
          const SourceWork& source_work = work[batch_begin + local_col];
          for (const Query& query : source_work.queries) {
            const float distance = dense_dist_at(result.dense_dist,
                                                graph.rows,
                                                static_cast<int>(batch_sources.size()),
                                                static_cast<int>(local_col),
                                                query.target);
        ...
        const double seconds =
            std::chrono::duration<double>(batch_end_time - batch_start_time).count();
        std::cout << "[bf_batched] finished batch " << batch_index << "/" << batch_count
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf_batched] warning: hipStreamDestroy during cleanup failed: "
    ...
    std::cout << "[bf_batched] wrote " << paths_written
              << " path records to " << output_path.string()
              << " reached=" << reached_count
              << " unreached=" << unreached_count << '\n';
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "[bf_batched] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
