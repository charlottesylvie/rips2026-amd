...
//     SSSP-new/src/bf7.cpp \
//     -o bf7
//
// Run summary-only; JSONL path output is disabled by default:
//   ./bf7 SSSP-new/data/logicnets_jscl.outgoing.csrbin \
//     SSSP-new/data/logicnets_jscl.outgoing.csrbin.ifmeta.bin \
//     bf7_summary.csv \
//     --source-progress-every 100
//
// Add --paths-output SSSP-new/paths/bf7.logicnets_jscl.outgoing.paths.jsonl
...
// Output: allocated workspace with GPU arrays and one pinned host status slot.
// Purpose: allocate frontier state, optional gather scratch, and reusable status handoff.
DeviceSsspWorkspace make_sssp_workspace(Offset rows,
  ...
  try {
    ...
    check_hip(hipHostMalloc(reinterpret_cast<void**>(&workspace.host_iteration_status),
                            sizeof(IterationStatus)),
              "hipHostMalloc SSSP pinned frontier status");
    ...
    if (workspace.host_iteration_status) (void)hipHostFree(workspace.host_iteration_status);
...
// GPU device helper. Inputs: next-frontier mark slot and current token.
// Output: true only for the thread that first marks this destination this iteration.
// Purpose: skip the unconditional atomic exchange once a vertex is already queued.
__device__ __forceinline__ bool mark_next_frontier_once(int* mark_addr,
                                                        int mark_token) {
  const int observed = *mark_addr;
  if (observed == mark_token) {
    return false;
  }
  return atomicCAS(mark_addr, observed, mark_token) == observed;
}
...
__device__ __forceinline__ void relax_outgoing_edge(
  ...
  if (atomic_relax_state(&best_state[static_cast<Offset>(dst)],
    ...
    if (mark_next_frontier_once(&next_marks[static_cast<Offset>(dst)], mark_token)) {
...
// Hybrid CPU/GPU host helper. Inputs: workspace and stream.
// Output: iteration status reset on the GPU using the pinned host status slot.
// Purpose: avoid a tiny status-reset kernel launch in every frontier iteration.
void reset_iteration_status_async(DeviceSsspWorkspace& workspace,
                                  hipStream_t stream = nullptr) {
  if (!workspace.iteration_status || !workspace.host_iteration_status) {
    throw std::runtime_error("SSSP frontier status workspace is not allocated");
  }
  *workspace.host_iteration_status = IterationStatus{};
  check_hip(hipMemcpyAsync(workspace.iteration_status,
                           workspace.host_iteration_status,
                           sizeof(IterationStatus),
                           hipMemcpyHostToDevice,
                           stream),
            "copy outgoing SSSP frontier status reset");
}

// Hybrid CPU/GPU host helper. Inputs: workspace and stream.
// Output: synchronized iteration status copied through pinned host memory.
// Purpose: keep the per-iteration GPU->CPU handoff small and reusable.
IterationStatus copy_iteration_status_to_host(DeviceSsspWorkspace& workspace,
                                              hipStream_t stream = nullptr) {
  if (!workspace.iteration_status || !workspace.host_iteration_status) {
    throw std::runtime_error("SSSP frontier status workspace is not allocated");
  }
  ...
  return *workspace.host_iteration_status;
}
...
OutgoingSsspResult run_outgoing_frontier_sssp_distance_early_stop(
  ...
  for (int iter = 0; iter < max_iters && frontier_count > 0; ++iter) {
    ...
    reset_iteration_status_async(workspace, stream);
    ...
    const IterationStatus status = copy_iteration_status_to_host(workspace, stream);
...
// Purpose: skip metadata sections bf7 does not need.
...
// Purpose: load the outgoing CSR format consumed by bf7's frontier kernels.
...
// Purpose: decode bf7's packed predecessor state on the host.
...
// Purpose: decode the low 32 bits used by bf7's frontier kernels.
  ...
  for (Offset from = 0; from < graph.rows; ++from) {
    for (Offset edge = graph.rowptr[static_cast<std::size_t>(from)];
      ...
      if (graph.edge_id[static_cast<std::size_t>(edge)] != edge) {
        throw std::runtime_error(
            "bf7 path output requires outgoing CSR edge_id[k] == k");
      }
...
// Purpose: document bf7's path-outputting command line.
...
int main_impl(int argc, char** argv) {
  ...
  std::cout << "[bf7] loading outgoing CSR: " << args.csr_path.string() << '\n';
  HostOutgoingCsrF32 graph = load_outgoing_csrbin(args.csr_path);
  std::cout << "[bf7] graph rows=" << graph.rows
            << " nnz=" << graph.nnz
            << " orientation=outgoing(row u stores edges u -> v)\n";

  std::cout << "[bf7] loading metadata: " << args.metadata_path.string() << '\n';
  ...
  std::cout << "[bf7] raw_metadata_sources=" << work.raw_source_count
  ...
  const Offset target_capacity =
      args.options.distance_early_stop
          ...
          : 0;
  std::cout << "[bf7] distance_early_stop="
  ...
  if (emit_paths) {
    ...
    std::cout << "[bf7] writing paths JSONL: "
              << args.paths_output_path.string() << '\n';
  } else {
    std::cout << "[bf7] paths JSONL disabled; pass --paths-output <path> to emit paths\n";
  }
  if (args.options.print_frontiers) {
    std::cout << "[bf7] frontier printing enabled for detailed source progress lines";
  ...
  if (source_count == 0) {
    ...
    std::cout << "[bf7] no valid unique sources to run\n";
    if (emit_paths) {
      paths_out.flush();
      if (!paths_out) {
        throw std::runtime_error("failed while writing paths JSONL");
      }
      std::cout << "[bf7] wrote paths: " << args.paths_output_path.string() << '\n';
    }
    return 0;
  }
  ...
  try {
    ...
    std::cout << "[bf7] copying outgoing CSR to GPU once\n";
    d_graph = copy_outgoing_csr_to_device(graph, stream);
    graph_copied = true;

    std::cout << "[bf7] allocating reusable frontier SSSP workspace\n";
    ...
    for (std::size_t source_index = 0; source_index < source_count; ++source_index) {
      ...
      if (detailed) {
        ...
        std::cout << "[bf7] source " << completed << "/" << source_count
                  << " node=" << source
        ...
        std::cout << " elapsed_sec=" << std::fixed << std::setprecision(3)
                  ...
                  << sources_per_sec << '\n';
        if (args.options.print_frontiers) {
          std::cout << "[bf7] frontier_trace source " << completed << "/"
                    << source_count
                    << " node=" << source
                    << " iterations_used=" << result.iterations_used
                    << " frontiers=";
    ...
    std::cout << "[bf7] termination_counts"
    ...
    if (emit_paths) {
      ...
      std::cout << "[bf7] path_records=" << paths_written
    ...
    if (stream_created) {
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf7] warning: hipStreamDestroy during cleanup failed: "
  ...
  std::cout << "[bf7] completed " << source_count
            << " source SSSP runs"
            << " total_elapsed_sec=" << std::fixed << std::setprecision(3)
            << total_seconds << '\n';
  if (!args.output_path.empty()) {
    std::cout << "[bf7] wrote summary: " << args.output_path.string() << '\n';
  }
  if (emit_paths) {
    std::cout << "[bf7] wrote paths: " << args.paths_output_path.string() << '\n';
  }
...
// CPU function. Inputs: command-line arguments. Output: process exit code.
// Purpose: top-level exception boundary for the bf7 executable.
int main(int argc, char** argv) {
  try {
    return main_impl(argc, argv);
  } catch (const std::exception& ex) {
    std::cerr << "[bf7] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
