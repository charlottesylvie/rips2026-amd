...
//     SSSP-new/src/bf3.cpp \
//     -o bf3
//
// Run summary-only; JSONL path output is disabled by default:
//   ./bf3 SSSP-new/data/logicnets_jscl.outgoing.csrbin \
//     SSSP-new/data/logicnets_jscl.outgoing.csrbin.ifmeta.bin \
//     bf3_summary.csv \
//     --source-progress-every 100
//
// Add --paths-output SSSP-new/paths/bf3.logicnets_jscl.outgoing.paths.jsonl
...
// Hybrid CPU/GPU host wrapper. Inputs: graph, workspace, source, max iterations, stream.
// Output: iteration count and convergence flag; final packed states remain in workspace.
// Purpose: run active-frontier Bellman-Ford until no vertices improve.
...
// Purpose: skip metadata sections bf3 does not need.
...
// Purpose: load the outgoing CSR format consumed by bf3's frontier kernels.
...
// Purpose: decode bf3's packed predecessor state on the host.
...
// Purpose: decode the low 32 bits used by bf3's frontier kernels.
  ...
  for (Offset from = 0; from < graph.rows; ++from) {
    for (Offset edge = graph.rowptr[static_cast<std::size_t>(from)];
      ...
      if (graph.edge_id[static_cast<std::size_t>(edge)] != edge) {
        throw std::runtime_error(
            "bf3 path output requires outgoing CSR edge_id[k] == k");
      }
...
// Purpose: document bf3's path-outputting command line.
...
int main_impl(int argc, char** argv) {
  ...
  std::cout << "[bf3] loading outgoing CSR: " << args.csr_path.string() << '\n';
  HostOutgoingCsrF32 graph = load_outgoing_csrbin(args.csr_path);
  std::cout << "[bf3] graph rows=" << graph.rows
            << " nnz=" << graph.nnz
            << " orientation=outgoing(row u stores edges u -> v)\n";

  std::cout << "[bf3] loading metadata: " << args.metadata_path.string() << '\n';
  ...
  std::cout << "[bf3] raw_metadata_sources=" << work.raw_source_count
  ...
  if (emit_paths) {
    ...
    std::cout << "[bf3] writing paths JSONL: "
              << args.paths_output_path.string() << '\n';
  } else {
    std::cout << "[bf3] paths JSONL disabled; pass --paths-output <path> to emit paths\n";
  }

  if (source_count == 0) {
    print_source_progress(0, 0, true);
    std::cout << "[bf3] no valid unique sources to run\n";
    if (emit_paths) {
      paths_out.flush();
      if (!paths_out) {
        throw std::runtime_error("failed while writing paths JSONL");
      }
      std::cout << "[bf3] wrote paths: " << args.paths_output_path.string() << '\n';
    }
    return 0;
  }
  ...
  try {
    ...
    std::cout << "[bf3] copying outgoing CSR to GPU once\n";
    d_graph = copy_outgoing_csr_to_device(graph, stream);
    graph_copied = true;

    std::cout << "[bf3] allocating reusable frontier SSSP workspace\n";
    ...
    for (std::size_t source_index = 0; source_index < source_count; ++source_index) {
      ...
      const rips_sssp_new::OutgoingSsspResult result =
          rips_sssp_new::run_outgoing_frontier_sssp(d_graph.view,
                                                    workspace,
                                                    source,
      ...
      if (detailed) {
        ...
        std::cout << "[bf3] source " << completed << "/" << source_count
                  << " node=" << source
    ...
    if (emit_paths) {
      ...
      std::cout << "[bf3] path_records=" << paths_written
    ...
    if (stream_created) {
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf3] warning: hipStreamDestroy during cleanup failed: "
  ...
  std::cout << "[bf3] completed " << source_count
            << " source SSSP runs"
            << " total_elapsed_sec=" << std::fixed << std::setprecision(3)
            << total_seconds << '\n';
  if (!args.output_path.empty()) {
    std::cout << "[bf3] wrote summary: " << args.output_path.string() << '\n';
  }
  if (emit_paths) {
    std::cout << "[bf3] wrote paths: " << args.paths_output_path.string() << '\n';
  }
...
// CPU function. Inputs: command-line arguments. Output: process exit code.
// Purpose: top-level exception boundary for the bf3 executable.
int main(int argc, char** argv) {
  try {
    return main_impl(argc, argv);
  } catch (const std::exception& ex) {
    std::cerr << "[bf3] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
