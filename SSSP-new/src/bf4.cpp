// Outgoing-CSR Bellman-Ford driver using a local bounded jump-frontier
// relaxation with optional validator-compatible source-to-sink path JSONL.
//
// Build from rips2026-amd on an AMD ROCm/HIP machine:
//   hipcc -std=c++17 -O3 -x hip \
//     SSSP-new/src/bf4.cpp \
//     -o bf4
//
// Run summary-only; JSONL path output is disabled by default:
//   ./bf4 SSSP-new/data/logicnets_jscl.outgoing.csrbin \
//     SSSP-new/data/logicnets_jscl.outgoing.csrbin.ifmeta.bin \
//     bf4_summary.csv \
//     --jump-k 2 \
//     --source-progress-every 100
//
// Add --paths-output SSSP-new/paths/bf4.logicnets_jscl.outgoing.paths.jsonl
...
// Purpose: run bounded jump-frontier Bellman-Ford until no vertices improve.
OutgoingSsspResult run_outgoing_jump_frontier_sssp(const DeviceOutgoingCsrF32& graph,
                                                   DeviceSsspWorkspace& workspace,
                                                   int source,
                                                   int max_iters,
                                                   int jump_k,
  ...
  if (jump_k <= 0) {
    throw std::runtime_error("jump_k must be positive");
  }
  if (max_iters < 0) {
  ...
  for (int iter = 0; iter < max_iters && frontier_count > 0; ++iter) {
    for (int depth = 0; depth < jump_k && frontier_count > 0; ++depth) {
      ...
      if (mark_token == std::numeric_limits<int>::max()) {
        check_hip(hipMemsetAsync(workspace.next_marks,
                                 0,
                                 static_cast<std::size_t>(workspace.rows) * sizeof(int),
                                 stream),
                  "reset outgoing SSSP frontier marks after token wrap");
        mark_token = 0;
      }
      ++mark_token;

      // Each jump depth is a separate kernel launch. The status copy and
      // stream sync below provide the global synchronization point needed
      // before launching the next micro-frontier depth.
      ...
      check_hip(hipStreamSynchronize(stream), "synchronize outgoing SSSP frontier jump depth");
...
// Purpose: skip metadata sections bf4 does not need.
...
// Purpose: load the outgoing CSR format consumed by bf4's frontier kernels.
...
// Purpose: decode bf4's packed predecessor state on the host.
...
// Purpose: decode the low 32 bits used by bf4's frontier kernels.
  ...
  for (Offset from = 0; from < graph.rows; ++from) {
    for (Offset edge = graph.rowptr[static_cast<std::size_t>(from)];
      ...
      if (graph.edge_id[static_cast<std::size_t>(edge)] != edge) {
        throw std::runtime_error(
            "bf4 path output requires outgoing CSR edge_id[k] == k");
      }
...
// Purpose: document bf4's path-outputting command line.
void print_usage(const char* program) {
  std::cerr
      ...
      << "  --jump-k <n>                Frontier micro-steps per outer iteration. Default: 1.\n"
...
ParsedArgs parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--max-iters") {
      ...
      parsed.options.jump_k =
          parse_int(require_value(&i, argc, argv, "--jump-k"), "--jump-k");
  ...
  if (parsed.options.jump_k <= 0) {
    throw std::runtime_error("--jump-k must be positive");
  }
  return parsed;
}
...
int main_impl(int argc, char** argv) {
  ...
  std::cout << "[bf4] loading outgoing CSR: " << args.csr_path.string() << '\n';
  HostOutgoingCsrF32 graph = load_outgoing_csrbin(args.csr_path);
  std::cout << "[bf4] graph rows=" << graph.rows
            << " nnz=" << graph.nnz
            << " orientation=outgoing(row u stores edges u -> v)\n";

  std::cout << "[bf4] loading metadata: " << args.metadata_path.string() << '\n';
  ...
  std::cout << "[bf4] raw_metadata_sources=" << work.raw_source_count
  ...
  std::cout << "[bf4] jump_k=" << args.options.jump_k
            << " frontier micro-step"
            << (args.options.jump_k == 1 ? "" : "s")
            << " per outer iteration\n";
  ...
  if (emit_paths) {
    ...
    std::cout << "[bf4] writing paths JSONL: "
              << args.paths_output_path.string() << '\n';
  } else {
    std::cout << "[bf4] paths JSONL disabled; pass --paths-output <path> to emit paths\n";
  }

  if (source_count == 0) {
    print_source_progress(0, 0, true);
    std::cout << "[bf4] no valid unique sources to run\n";
    if (emit_paths) {
      paths_out.flush();
      if (!paths_out) {
        throw std::runtime_error("failed while writing paths JSONL");
      }
      std::cout << "[bf4] wrote paths: " << args.paths_output_path.string() << '\n';
    }
    return 0;
  }
  ...
  try {
    ...
    std::cout << "[bf4] copying outgoing CSR to GPU once\n";
    d_graph = copy_outgoing_csr_to_device(graph, stream);
    graph_copied = true;

    std::cout << "[bf4] allocating reusable frontier SSSP workspace\n";
    ...
    for (std::size_t source_index = 0; source_index < source_count; ++source_index) {
      ...
      const rips_sssp_new::OutgoingSsspResult result =
          rips_sssp_new::run_outgoing_jump_frontier_sssp(d_graph.view,
                                                         workspace,
                                                         source,
      ...
      if (detailed) {
        ...
        std::cout << "[bf4] source " << completed << "/" << source_count
                  << " node=" << source
    ...
    if (emit_paths) {
      ...
      std::cout << "[bf4] path_records=" << paths_written
    ...
    if (stream_created) {
      ...
      if (destroy_status != hipSuccess) {
        std::cerr << "[bf4] warning: hipStreamDestroy during cleanup failed: "
  ...
  std::cout << "[bf4] completed " << source_count
            << " source SSSP runs"
            << " total_elapsed_sec=" << std::fixed << std::setprecision(3)
            << total_seconds << '\n';
  if (!args.output_path.empty()) {
    std::cout << "[bf4] wrote summary: " << args.output_path.string() << '\n';
  }
  if (emit_paths) {
    std::cout << "[bf4] wrote paths: " << args.paths_output_path.string() << '\n';
  }
...
// CPU function. Inputs: command-line arguments. Output: process exit code.
// Purpose: top-level exception boundary for the bf4 executable.
int main(int argc, char** argv) {
  try {
    return main_impl(argc, argv);
  } catch (const std::exception& ex) {
    std::cerr << "[bf4] ERROR: " << ex.what() << '\n';
    return 2;
  }
}
